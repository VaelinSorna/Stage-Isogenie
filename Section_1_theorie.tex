...
\begin{Def}
	Soit $\Lambda$ un réseau. On note $E_{\Lambda}$ le modèle de courbe elliptique d'équation $y^{2} = 4x^{2} - g_2 x - g_3$.
\end{Def}
...
\subsection{Réduction de courbes elliptiques et relèvement de Deuring}

Dans cette section, on généralise les résultats précédents aux courbes elliptiques ordinaires, sur des corps finis. Plusieurs problèmes se posent. L'équivalence de catégories avec les tores complexes ne se généralise pas directement, et il n'y a pas de bijection entre les endomorphismes et leurs facteurs puisque certains seront inséparables donc de facteurs nuls. Pour palier à cela, on énonce des résultats de réduction et de relèvement de courbes elliptiques. Soit $E$ une courbe elliptique à multiplication complexe définie sur $\Qb$, et $p$ un nombre premier. On veut construire une courbe elliptique $E_p$ définie sur un corps fini $\F_{q}$ de caractéristique $p$, telle que $End(E)\simeq End(E_p)$.

\begin{Rem}
	La théorie générale de la réduction de courbes elliptiques utilise la notion de corps de nombres $p$-adiques. Cependant pour les courbes elliptiques à multiplication complexe, le fait de toujours être définie sur un corps de nombres permet de simplifier les énoncés. On n'énoncera pas la théorie générale et on admettra les résultats 
\end{Rem}  

\begin{Def}
	On note $\Zb$ l'anneaux des entiers algébriques. Si $\PR$ est un idéal premier de $\Zb$, on dira que $\PR$ est au dessus d'un nombre premier $p$ si $\PR\cap\Z = p\Z$. 
\end{Def}
...
Il se peut que des points finis de $E$ se réduise sur le point à l'infini de $E_\PR$. Cependant cela n'arrivera pas pour les points d'ordre fini de $E$ premier avec la caractéristique $p$.
...
Dans le cas d'une réduction complète, on peut étendre la normalisation de $End(E)$ à $End(E_\PR)$ en réduisant le facteur modulo $\PR$.
...
\begin{Prop}[\cite{Lang1987}(9 \S4)]
	...
	\end{equation*}
	En particulier $\widetilde{[a]}$ est inséparable si et seulement si $\alpha\in\PR$
\end{Prop}

\begin{Rem}
	D'après la proposition précédente, la notation $\left[ \tilde{\alpha} \right]$ n'a donc pas de sens par il existe plusieurs endomorphisme de $E_\PR$ de facteur $\tilde{\alpha}$.
\end{Rem}

\begin{Prop}[\cite{Lang1987}(Cor 13.5)]\label{DescriptionFrobReduction}
	
	Soit $E\in Ell_{\Qb}(\OR_\Delta)$, $End(E)$ étant un ordre du corps $\K$. On suppose que $E$ se réduit complètement modulo $\PR\subset\Zb$. Ainsi existe un corps finis $\F_{q}$ sur lequel $E_\PR$ est défini (voir la remarque \ref{RemarqueReductionCorpsFinis}), et la caractéristique $p$ est décomposé dans $\OR_{\Delta}$. Soit $\pi_{q}$ l'endomorphisme de Frobenius, et $\alpha\in\OR_{\Delta}$ tel que $\widetilde{[\alpha]} = \pi_{q}$. Alors $\alpha\in \PR\cap\OR_{\Delta}$. De plus l'un des deux idéaux au dessus de $p$ dans $\OR_{\Delta}$ est $\PR\cap\OR_{\Delta}$. Enfin si $q = p^d$, $\alpha\OR_{\Delta} = \left(\PR\cap\OR_{\Delta} \right) ^{d}$
...
On peut de plus voir les éléments de $Ell_{\F_{q}}(\OR_{\Delta})$ comme des réductions modulo même idéal maximal $\PR\in\Zb$. 
...
\begin{Rem}
	Ainsi l'injectivité de la réduction des isogénies (proposition \ref{reductionIsogenieInjectivité}) permet d'associer à toute isogénie entre courbes de $Ell_{\Qb}(\OR_{\Delta})$ une isogénie entre leurs courbes réduites de $Ell_{\F_{q}}(\OR_{\Delta})$. On rappelle cependant que l'on n'a pas la surjectivité au niveau des isogénies à priori.
\end{Rem}

On peut expliciter le noyau d'une isogénie réduite à partir d'un idéal entier propre $\LR\subset\OR_{\Delta}$ en généralisant la notion de groupe de torsion.
...
\subsection{Représentation de l'endomorphisme de Frobenius}
...
\textbf{TODO : def ou remarque, definition canonique d'une normalisation de End(E)}
...
\begin{Prop}
	...
	Alors $\alpha\in\C$ une racine complexe du polynôme caractéristique de $E$. Soit $\OR_{\K} = \left[ 1, w_{\K}\right] $, de sorte que $\OR_{\Delta} = \left[ 1, fw_{\K}\right] $ et $Z[\alpha] = \left[ 1, f_{m}w_{\K}\right] $. Alors il existe un entier $s_{\alpha}$ tel que $\pm\alpha = f_{m}w_{\K} + s_{\alpha}$
...
\begin{Rem}
	D'après le théorème de relèvement de During, il existe une courbe $\left( E',\left[ .\right] \right) $ définie sur $\C$ à multiplication complexe, munie d'un endomorphisme $\left[ \alpha\right] $ et d'un idéal maximal $\PR\subset\Zb$, de sorte que $E'_PR = E$ et $\tilde{\alpha} = \pi_{q}$. Alors $\alpha$ est l'une des deux racines complexes du polynôme caractéristique de $E$. Sa valeur dépend de l'idéal de réduction $\PR$ : d'après la proposition \ref{DescriptionFrobReduction}, 
	\begin{equation*}
		\alpha\OR_{\Delta} = \left(\PR\cap\OR_{\Delta} \right) ^{d}
	\end{equation*} 
\end{Rem}
...
\begin{proof}
	Ces résultats découlent de l'égalité $\pm w_{\K} = \frac{\pi_{q} - s}{f_{m}}$ où $s\in\Z$.
\end{proof}
