...
\subsubsection{Rappels et notations}

On suppose connues les notions de théorie des courbes elliptiques présentées dans le cours du master. On énonce ici les définitions et notations utilisées dans la suite. Dans la suite, on énonce des résultats généraux sur les courbes elliptiques découverts et étudiés pendant le stage. 
...
\begin{Rem}
	On considère, sauf mention contraire, des modèles de courbes elliptiques donnés par des équation de Weierstrass réduites $E : y^2 = x^3 + ax + b$. On notera $\OR$ le point neutre, ou $\OR_{E}$ s'il y a besoin de précision (pour différentier des ordres par exemple). 
\end{Rem}
...
\begin{Rem} On sait que l'extension $\Kb(C) / \Kb(x)$ est fini pour tout $x\in\Kb(C)$ non constant (d'après le théorème de normalisation de Noether, vu dans le cours de Courbes algébriques). En caractéristique 0, $dx$ est donc toujours une base de $Omega_{C}$ car la séparabilité de l'extension est garantie. 
\end{Rem}
...
\begin{Prop}\label{ActionClassesSTrans}
...
\subsubsection{Groupes de torsions d'idéaux entiers}
...
\begin{Prop}\label{IsogénieIdealEntierIdentité}
...
\subsubsection{Réduction de courbe elliptique et relèvement de Deuring}

Dans cette section, on souhaite généraliser les résultats précédents aux courbes elliptiques ordinaires, sur des corps finis. Plusieurs problèmes se posent. L'équivalence de catégories avec les tores complexes ne se généralise pas directement, et les isogénies inséparables seront de facteurs nuls. On énonce des résultats de réduction et de relèvement de courbes elliptiques, permettant de définir des isogénies à partir d'idéaux propres

\textbf{TODO : ajouter structure de volcan et théorie sur corps finis. ici ou avant}
