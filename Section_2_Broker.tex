...
\begin{enumerate}
	...
	\item D'après l'hypothèse $\ref{HypothèseInversion}$, $\Psi_{l}(j,Y)$ admet exactement deux racines dans $\F_{q}$. On souhaite trouver l'unique isogénie normalisée de noyau $E[\LR]$. Il faut donc tester si le polynôme $F$ obtenue par BMSS décrit le bon noyau. Pour cela on vérifie formellement la valeur propre avec laquelle agit le morphisme de Frobenius sur l'ensemble de points décrit par $F$, comme énoncé dans la proposition \ref{propositionCheckElkies}.
...
\begin{Prop}\label{propositionCheckElkies}
	...
	\end{equation*}
	Soit $(R_{1}(X), YR_{2}(X))$ les fonctions rationnelles définissant l'endomorphisme $[-ck]$. Alors $F$ est le polynôme décrivant $E\left[ \LR\right]$ si et seulement si, dans le corps de fraction de $R$, on a:
	\begin{equation*}
...
\begin{Rem}
	Dans toute la suite, pour vérifier si un polynôme donné décrit le bon noyau, on utilise l'algorithme CheckElkies de l'implémentation Sage de \cite{Pegasis}. J'ai aussi essayer d'implémenter cette algorithme, mais ma version n'est pas aussi bien optimisé.  
\end{Rem}
...
\begin{algorithm}[h]
	...
	\begin{algorithmic}[1]
		\Require $E\in Ell_{\F_{q}}(\OR_{\Delta})$. un entier positif $n$ et un nombre premier $l$ vérifiant l'hypothèse \ref{HypothèseInversion}. Un idéal $\LR\subset\OR_{\Delta}$ de norme $l$. Un point $P\in E(\F_{q^{n}})$. Un idéal $\LR'$ friable équivalent à $\LR$, $L_{facteurs}$ la liste des idéaux premier de la factorisation de $\LR'$, et $L_{exposants}$ la liste des exposants associés.
...
\begin{Prop}[\cite{Bostan_2008}]
	...
	Soit $M : \N\times\N\rightarrow\N$ une fonction telle que $M(n,q)$ soit la complexité de la multiplication de deux polynômes de degrés au plus $n$ dans $\F_{q}[X]$. L'algorithme BMSS calcule le polynôme décrivant le noyaux d'une isogénie définie sur $\F_{q}$ de degré $p_{i}$ avec une complexité en 
	\begin{equation*}
...
\begin{Rem}
	Pour estimer la constante $C$, Seysen \cite{Seysen} cite deux résultats proposant $C= 280$ ou même $C = 2$. Cohen \cite{Coh00} propose $C = 6$. Cependant d'après Cohen, en pratique cette borne $N$ est pessimiste et l'on conjecture une borne en $O(\log(\Delta)^{1+\epsilon})$ pour tout $\epsilon > 0$. On choisira $C=2$ et on supposera $p$ suffisamment grand pour que les nombres premiers inférieurs à $N$ vérifient l'hypothèse \ref{HypothèseElkies}.
\end{Rem}
...
\begin{proof}
	On parcourt tout les entiers $n$ plus petit que $N$, en testant s'ils sont premier en $O(\log(n)^4)$ (Algorithme de Miller-Rabin). Pour chaque nombre premier $n$ regardé, le calcul intermédiaire le plus coûteux est celui d'une racine $b_{n}$ de $\Delta$ modulo $4n$, en $O(\log(n)^4)$ (Algorithme de Tonelli et Shanks \cite{Coh00}[1.5.1]). Le nombre de nombres premiers inférieurs à $N$ est approché par $N/\log(N)$. D'où une complexité globale majorée en $O(N\log(N)^7)$.
\end{proof}
...
\begin{Def}
	...
	\end{equation*}
	On dira qu'une procédure est sous-exponentielle en $\log(|P|)$ s'il existe des constante $\alpha$, $z$ telles que sa complexité soit en $O(L_P(\alpha,z))$. 
...
\begin{proof}
	Soit $t$ le nombre de pas de la marche aléatoire de l'algorithme \ref{ALGO FactoClasse}, choisit selon la proposition \ref{espéranceExposant}. Soit $k$ la taille des listes retournées par l'algorithme \ref{ALGO FamilleGénératrice}. D'après la remarque \ref{comportementListeExposant}, le calcul de $classe_test$ lors ds lignes $13$ à $18$ demande $t$ produits, chacun entre une forme réduite et une forme première associée à un nombre premier $p_{i} \leq N$, et à chaque fois suivi d'une réduction par l'algorithme \ref{ALGO reduction}. En majorant les coefficients d'une forme réduite $(a,b,c)$ selon la proposition \ref{tailleCoefReduit},
...
On déduit de ce qui précède la complexité de l'algorithme \ref{ALGO FactoClasse}. On énonce deux proposition selon si l'on choisit $N_t$ ou $N_p$, pour prouver l'avantage asymptotique de l'optimisation proposée par \cite{JaoSoukharev} en choisissant $N_p$. 
...
\begin{proof}
	...
	\begin{equation*}
		O\left( \exp \left(  \log\log\left(|\Delta| \right)  \frac{\log\left(|\Delta| \right)}{4\log(\log(|\Delta|))} \right) \right) = O\left( \sqrt[4]{|\Delta|}\right).
	\end{equation*}
...
\begin{Rem}\label{choixOptimalN}
	Jao et Soukharev \cite{JaoSoukharev} proposent de choisir $N = N_p$. Cependant d'après Cohen \cite{Coh00}[5.5.1], même si la borne théorique est asymptotiquement plus petite que la borne pratique, pour des discriminants $\Delta < 10^{100}$ c'est la borne sous-exponentielle qui sera plus petite. $N_p$ n'est alors pas suffisamment grand a priori pour que l'algorithme \ref{ALGO FamilleGénératrice} appliqué à $N = N_p$ retourne une famille génératrice. De plus lorsque l'on choisit $N = N_t$, la complexité exponentielle de l'algorithme \ref{ALGO FactoClasse} est dû au fait que $N_t$ est trop petit, ce qui implique une probabilité de réussite trop faible lors des marches aléatoires. Le choix de $N$ optimal est donc
	\begin{equation*}
		N = \max(N_p,N_t).
	\end{equation*}
...
\begin{Prop}\label{COMPLEX equivalenceFriablePostFactoClass}
	On considère que l'entrée $\LR'$ de l'algorithme \ref{ALGO IsogenieEquivalenceFriable} est issue de l'algorithme \ref{ALGO FactoClasse} appliquée avec des paramètre $N$ et $t$. Alors la complexité de  l'algorithme \ref{ALGO IsogenieEquivalenceFriable} est en 
	\begin{equation*}
		O\left( t\left(N^3\log(N)^{2} + N^2\log(q) \right) + tN^2\log(q^n) + \left( \log(l) + t\log(N)\right)^{2}\right). 
	\end{equation*}
...
\begin{proof}
	...
	\end{equation*}
	Le coûts des manipulations de $\Psi_{p_{i}}$ (dérivations partielles, évaluations) ont un coup estimé par \cite{JaoSoukharev} en 
	\begin{equation*}
	...
	\end{equation*}
	Ensuite les évaluations de chaque isogénies nécessitent de mener des calculs dans le corps $\F_{q^{n}}$, car $P\in E(\F_{q^{n}})$. L'évaluation d'une isogénie de degré $p_{i}$ obtenue avec les formules de Vélu et l'algorithme BMSS nécessite $O(p_{i}^{2})$ opérations dans $\F_{q^{n}}$ (d'après \cite{JaoSoukharev}), chacune de complexité en $O(\log(q^n))$. On évalue au total en moyenne $t$ isogénies. En majorant $p_{i}$ par $N$, on obtient une complexité en 
	\begin{equation*}
		O(tN^2\log(q^n)). 
	\end{equation*}

	La norme de l'idéal $\LR_{principal}$ calculé lors de la boucle For des lignes $5$ à $15$ de l'algorithme \ref{ALGO IsogenieEquivalenceFriable} est en $O(lN^t)$. Le calcul de $\LR_{principal}$ est en $O\left( \left( log(l) + tlog(N)\right) \right)$. La complexité de la recherche du générateur est en 
	\begin{equation*}
		O\left( \left( \log(l) + t\log(N)\right)^{2} \right). 
	\end{equation*}
	Enfin la complexité des calculs de la forme standard $\LR_{principal}$ et de son générateur dépendent de sa norme. Puisque le nombre d'isogénie calculé est en moyenne égale à $t$, $N\left(\LR_{principal} \right) \leq lN^t$. La complexité du calcul du produit de deux idéaux entier $\IR$ et $\LR$ donnés sous forme standard est en $O\left( \max\left( N(\IR),N(\LR)\right) \right)$ (d'après \cite{JaoSoukharev}), donc la complexité du calcul de $\LR_{principal}$ peut être majorée en 
	\begin{equation*}
		O\left(\log\left(N\left(\LR_{principal}\right)\right)\right) = O\left( \left( \log(l) + t\log(N)\right) \right).
	\end{equation*} 
	La complexité de l'algorithme \ref{ALGO reduction} appliqué à $\LR_{principal}$ est majorée par
	\begin{equation*}
		 O\left( \log\left(N\left(\LR_{principal} \right)  \right)^2 \right) = O\left( \left( \log(l) + t\log(N)\right)^{2} \right).
	\end{equation*}
\end{proof}



On déduit de ce qui précède la complexité de l'algorithme. on énonce les résultats obtenues selon les choix possibles pour le paramètre $N$. 


\begin{Coro}
	On considère que l'entrée $\LR'$ de l'algorithme \ref{ALGO IsogenieEquivalenceFriable} est issue de l'algorithme \ref{ALGO FactoClasse} appliquée avec des paramètres $N$ et $t$ choisit respectivement selon la remarque \ref{choixOptimalN} et la proposition \ref{espéranceExposant}. Alors la complexité de l'algorithme \ref{ALGO IsogenieEquivalenceFriable} est en
	\begin{equation*}
