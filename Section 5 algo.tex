...
\begin{Prop}
	Soit $N$ un entier positif, $\Delta$ un discriminant, et $A = pf_{m}|E(\F_{q^{n}})|$. L'algorithme $3$ renvoie une famille supposée génératrice du groupe de classe de norme premier bornée par $N$ avec une complexité en 
	\begin{equation*}
		O(N\log(N)^3).
	\end{equation*} 
\end{Prop}
...
\begin{proof}
	On parcourt tout les nombres premiers plus petit que $N$. La boucle de la ligne $3$ demande donc d'après le théorème des nombre premier $O(N/\log(N))$ passages. Pour chaque nombre premier $n$ regardé, le calcule intermédiaires les plus coûteux est le calcul d'une racine $b_{n}$ de $\Delta$ modulo $4n$, en $O(\log(n)^4)$ avec l'algorithme de Tonelli et Shanks \cite{Coh00}[1.5.1] (on peut faire plus rapide lorsque $n\neq 1$ modulo $8$, mais ceci est la meilleure méthode générale. L'implémentation Sage applique la méthode la plus rapide connue selon la valeur de $n$ modulo $16$.). D'où une complexité globale en $O(N\log(N)^3)$.
\end{proof}

\cite{JaoSoukharev} et \cite{Coh00}[\S 5.5.1] propose une astuce pour optimiser la complexité de l'algorithme 4 : Si $N$ est trop petit, on risque de ne pas avoir une famille génératrice, et il est plus difficile de trouver par hasard une classe facilement factorisable. Mais si $N$ est trop grand, l'algorithme $3$ prend plus de temps. Il est aussi plus lent de vérifié si un entier se factorise dans la famille de normes voulue. La borne théorique proposant $N = 2\log(|\Delta|)^{2}$ étant supposé pessimiste\cite{Coh00}[\S 5.5.1], on choisit $N$ différemment :

\begin{Def}
	Soit $\Delta$ un discriminant. On défini la complexité sous exponentielle en $\Delta$ par la fonction
	\begin{equation*}
		L(\alpha,z) = e^{\left( (z + O(1))\log^{\alpha}(|\Delta|)\log^{1-\alpha}(\log(|\Delta|))\right) }
	\end{equation*}
	On dira qu'une procédure est sous-exponentielle en $\Delta$ s'il existe des constante $\alpha$, $z$ telles que la complexité soit en $O(L(\alpha,z))$.
\end{Def}
...
\begin{Prop}
	Soit $t = C\log\left( |\Delta|\right) /\log\left( \log\left( |\Delta|\right) \right)$ le nombre de pas de la marche aléatoire. Soit $k$ la taille des listes retournées par l'algorithme $3$. Si $N = L(\frac{1}{2},z)$ et $t\leq k$, alors 
	\begin{enumerate}
		\item Une itération de la boucle WHILE des lignes $5$ à $14$ de l'algorithme $4$ termine en moyenne en un temps en $?$
		\item Le nombre d'itération de la boucle des lignes $5$ à $14$ de l'algorithme $4$ est en $?$
		\item l'algorithme $4$ a une complexité moyenne en $?$
	\end{enumerate}
\end{Prop}

\begin{proof}
	
	D'après le théorème des nombres premiers, si l'on suppose $k = O(N/log(N))$. En particulier
	\begin{equation*}
		\frac{t}{k} = C\frac{\log\left( |\Delta|\right)}{\log\left( \log\left( |\Delta|\right) \right)}\frac{2\log(\log(|\Delta|))}{2\log(|\Delta|)^2} = \frac{C}{\log(|\Delta|)}
	\end{equation*}
	$t/k = O(1)$ est de l'ordre d'une constante $C_{0}$. Si $t\leq k$, $C_{0} \leq 1$. 
	Durant la marche aléatoire, chaque indice est choisi en moyenne $t/k$ fois, donc moins d'une fois. On peut donc estimer en moyenne qu'à l'étape $6$ la liste $L_{x}$ ne contient que des $0$ et des $1$, et au plus $t$ $1$. $ideal_{test}$ est donc de norme $\leq lp_{k-t}\ldots p_{k} \leq lp_{k}^{t}$. Calculer sa forme réduite $classe_{test}$ se fait en $O(log(lp_{k}^{t})) = O(log(l) + tlog(N))$. Vérifié si $classe_{test}$ se factorise demande $O(k)$ test de divisibilité et division euclidienne, chacune sur un coefficient $a$ d'une forme quadratique réduite : $a < \Delta$. Au total les étapes $11$ et $12$ demandent $O(klog(|\Delta|))$ opérations dans $\Z$. Les étapes $13$ à $20$ sont élémentaires. Il reste à savoir combien d'essais pour $L_{x}$ doit être fait en moyenne avant de réussir le test de l'étape $11$. \textbf{Heuristique nombre friable ?}
\end{proof}
