...
On souhaite utiliser la théorie précédente pour établir une procédure de calcul d'isogénie plus rapide que la méthode naïve donnée par la formule de Vélu. Plus précisément on considère :
\begin{enumerate}
	\item Une courbe elliptique $E$ ordinaire, donnée par une équation de Weierstrass réduite définie sur un corps $\F_{q}$ de caractéristique $p$. On précalcule le nombre de point du modèle $|E(\F_{q})|$.
	\item L'anneau d'endomorphismes $End(E)\simeq\OR_{\Delta}$ donné par son discriminant $\Delta$. On connaît donc le corps quadratique imaginaire $\K$ et le conducteur $f$ de $\OR_{\Delta}$. On supposera de plus que l'isomorphisme $End(E)\simeq\OR_{\Delta}$ est normalisé. 
	\item En notant $\pi_{q}$ l'endomorphisme de Frobenius sur $E$, on considère $f_{m}$ le conducteur de l'ordre $\Z\left[ \pi_{q}\right]$ dans $\K$, appelé conducteur maximal. 
...
\begin{Def}
	Avec les notations précédentes, on pose $A = pf_{m}|E(\F_{q^{n}})|$.
\end{Def}

\begin{Hyp}
	On suppose que $E$, $l$ et $n$ sont choisis tel que $l$ ne divise pas $A$, que $l$ est décomposé dans $End(E)$ et que $f_{m}$ et $|E(\F_{q^{n}})|$ sont premier entre eux.
\end{Hyp}

Cette condition nous oblige à éviter certains nombres premiers. Elle a cependant plusieurs avantages :
\begin{enumerate}
	\item Si $l$ ne divise pas $f_{m}$, alors d'après la proposition 4.27 toutes les $l$-isogénies séparables sont horizontales. Comme $l$ est décomposé, il existe $2$ $l$-isogénies et leurs noyaux sont accessibles comme groupes de torsions d'idéaux de norme $l$. 
	\item On peut même représenter une isogénie uniquement par l'idéal qui engendre son noyau. Un tel idéal peut être donné d'après la proposition 4.25 sous la forme $\left[ l, \frac{c + d\pi_{q}}{f_{m}}\right]$ donc seulement déterminé par $l$, $c$ et $d$, trois entier qui en pratique seront de $\log(l)$ bits. On peut donc représenter une $l$-isogénie en $O(log(l))$ bits. 
	...
	\item Si $l$ ne divise pas $A$, alors $f_{m}$ et $|E(\F_{q^{n}})|$ sont inversibles modulo $l$. Le fait que $f_{m}$ soit inversible sera utile par la suite pour manipuler des endomorphisme de la forme $\left[ \frac{c + d\pi_{q}}{f_{m}}\right] $. Le fait que $|E(\F_{q^{n}})|$ soit inversible sera utile pour normaliser l'isogénie obtenue (Voir section 5.5, 5.6).
	\item Le fait que $f_{m}$ et $|E(\F_{q^{n}})|$ soit premier entre eux est cruciale pour la dernière étape de l'algorithme $9$ (Voir section 5.6 proposition 5.25). 
\end{enumerate}

On énonce d'abord une procédure naïve à partir de la formule de Vélu, à condition de savoir expliciter le générateur d'un groupe de torsion.

\begin{algorithm}
	\caption{Calcul d'une forme explicite normalisée de $l$-isogénie}
	\begin{algorithmic}[1]
		\Require Un modèle défini sur $\F_{q}$ de courbe elliptique $E$, le discriminant $\Delta$ de $End(E)$, un nombre $l$ premier. Un entier $n\in\N$ et un point $P\in E(\F_{q^{n}})$. $E$, $l$ et $n$ vérifie l'hypothèse 1. 
		\Ensure Une forme explicite normalisée d'une $l$-isogénie $\phi : E \rightarrow E'$ partant de $E$. l'image $\phi(P)$. 
		\State Factoriser $l\OR_{\Delta}$ avec le théorème de Kummer
		\State Choisir un des deux idéaux $\LR$ de norme $l$ obtenu
		\State Calculer un générateur $K$ du groupe de torsion $E\left[ \LR\right]$ (voir section 5.4)
		...
		\State \Return La forme explicite $\phi : E \rightarrow E'$ et $\phi(P)$
	\end{algorithmic}
	
\end{algorithm}


Cependant la formule de Vélu demande de parcourir le noyau entier : la procédure est linéaire en $l$, ce qui n'est pas satisfaisant. L'algorithme de Broker, Charles et Lauter consiste à améliorer l'algorithme 1 en se ramenant à des idéaux de petites normes grâce à l'action du groupe de classes $Cl(\OR_{\Delta})$ sur $Ell_{\F_{q}}(\OR_{\Delta})$. On présente les différentes étapes de la façon suivante :
...
\begin{enumerate}
	...
	\item La procédure de Broker Charles et Lauter sera énoncé en section 5.6, puis la complexité sera étudié en section 5.7. 
\end{enumerate}
...
\subsection{Calculs dans les groupes de classes d'idéaux}
...
\begin{enumerate}
	...
	\item On voudra calculer des isogénies associées aux idéaux représentant les générateurs d'une famille génératrice. Il faut donc que leurs normes vérifient les conditions fixées sur $l$ : On veut pouvoir choisir des représentants de classes de sorte que leurs normes soient des nombres premiers bien choisis. 
\end{enumerate}

Un outil va nous permettre de résoudre les deux problèmes en même temps : les formes quadratiques à coefficients entiers. 
...
On cherche à trouver un représentant d'une classe d'idéaux de $\OR_{\Delta}$ de façon systématique et pratique pour l'implémentation. Prendre un idéal primitif ne suffit pas : il peut y avoir plusieurs idéaux primitifs dans la même classe. 
...
\begin{Prop}\cite{Cox}[Th 7.7]
	...
	Avec les notation précédentes, $I(f*g) = I(f)I(g)$ et $F(\LR\LR') = F(\LR)*F(\LR')$. Les applications $I$ et $F$ induisent des isomorphismes entre groupes de classes.
\end{Prop}
...
On définit une notion de forme quadratique binaire réduite, qui aura l'avantage d'être unique dans chaque classe :
...
La preuve de la proposition 5.7 fournit un algorithme donnant l'élément réduit de la classe en $\OR(\log(\Delta)^{2} )$ à partir d'un représentant quelconque.
...
\begin{Rem}
	...
	\textbf{Completer la remarque avec une définition de l'action d'une matrice. Partie théorique ?}
...
La forme réduite sera adapté pour faire des calculs dans le groupe de classe. Pour le calcul d'isogénie, on voudra cependant trouver un système de représentant de normes vérifiant l'hypothèse 1. A priori l'idéal associé à la forme réduite ne convient pas. 

\begin{Def}
	Un entier $m\in\N$ est dit représenté par une forme quadratique binaire primitive $f$ s'il existe $s$, $t$ deux entiers tels que $m = f(s,t)$.
\end{Def}
...
Ainsi, toute classe d'idéaux possède un représentant premier avec la constante $A$ de l'hypothèse 1. Comme il y a deux choix d'idéaux de norme $p$ premier lorsque $p$ est décomposé, on va fixer une convention nous permettant de choisir. 
...
On veut former une famille génératrice de $Cl(\OR_{\Delta})$ définie avec un système de représentant formé uniquement d'idéaux de norme $l$ vérifiant l'hypothèse 1. Une idée naïve est d'établir la liste croissante des nombres premiers acceptables inférieur à une borne $N$. Pour $N$ suffisamment grand on peut espérer avoir assez de représentant de classes distinctes parmi les idéaux premier associés pour obtenir une famille génératrice.
...
Comme on peut choisir $N$ de taille asymptotiquement raisonnable, on énonce  l'algorithme 3 qui sera utilisé pour établir une liste de représentant d'une famille génératrice du groupe des classes d'idéaux. 


\begin{algorithm}
	\caption{Famille génératrice du groupe de classes}
	\begin{algorithmic}[1]
		\Require Un entier positif $N$, un discriminant $\Delta$, la caractéristique $p$, le conducteur maximal $f_{m}$, le cardinal $|E(\F_{q^{n}})|$.
		\Ensure Une liste $L_{premiers}$ de nombres premiers vérifiant l'hypothèse 1, la liste $L_{ideaux}$ de leurs idéaux premiers associés respectifs, la liste $L_{formes}$ de leurs formes premières associées respectives. 
...
Soit $\LR\subset\OR_{\Delta}$ un idéal entier. Soit $L_{premiers}$, $L_{ideaux}$ et $L_{formes}$ les listes retournées par l'algorithme $3$. On note $k$ leurs tailles, et $ L_{ideaux} = \left\lbrace \PR_{1}\ldots\PR_{k}\right\rbrace $. Alors il existe $e_{1},\ldots,e_{k}\in\Z$ tels que
\begin{equation*}
	\CL = \CP_{1}^{e_{1}}\CP_{2}^{e_{2}}\ldots\CP_{k}^{e_{k}}
\end{equation*}
De plus il existe $\alpha\in\K$ tel que
\begin{equation*}
	\LR = (\alpha\OR_{\Delta})\PR_{1}^{e_{1}}\PR_{2}^{e_{2}}\ldots\PR_{k}^{e_{k}}
\end{equation*}
...
\begin{Coro}
	On reprend les notations du théorème précédent. Si $a$ se factorise entièrement avec des premiers $p_{i}\in L_{premiers}$ obtenue par l'algorithme 3, alors $\CL$ est facilement factorisable connaissant la factorisation de $a$ grâce au point 3 du théorème 5.11. 
\end{Coro}
...
\begin{Rem}
	Concrètement, dès que l'on enchaîne des marches aléatoires de taille de l'ordre de $\frac{\log(|\Delta|)}{\log(\log(|\Delta|))}$, les résultats seront uniformément répartis parmi les éléments de $Cl(\OR_{\Delta})$. Puisque le groupe est abélien, la norme $l_{1}$ du vecteurs des exposants, que l'on notera $x$, est au pire le nombre de pas $t$ de la marche aléatoire. 
\end{Rem}

On en déduit l'algorithme $4$ permettant de trouver les exposants $e_{i}$ cherché. L'idée est de choisir $S_{0}$ dans la proposition 5.13 comme étant l'ensemble des classes que l'on sait factoriser grâce au théorème 5.11. 
...
Dans cette partie, on suppose connaître une factorisation de la classe $\CL$ obtenue avec l'algorithme 4, c'est a dire
\begin{equation}
	\CL = \CP_{p_{1}}^{e_{1}}\CP_{p_{2}}^{e_{2}}\ldots\CP_{p_{g}}^{e_{g}},
\end{equation}
où les idéaux $\PR_{p_{i}}$ sont ceux de la liste $L_{facteurs}$ retourné par l'algorithme 4. On cherche alors $\alpha\in\K$ tel que 
\begin{equation}
	\LR = (\alpha\OR_{\Delta})\PR_{p_{1}}^{e_{1}}\PR_{p_{2}}^{e_{2}}\ldots\PR_{p_{g}}^{e_{g}}.
\end{equation}
On commence par se ramener à chercher un élément de $\OR_{\Delta}$. D'après la proposition 1.17, l'idéal conjugué $\bar{\PR_{i}}$ de $\PR_{i}$ est un représentant de $\CP_{p_{i}}^{-1}$, donc
\begin{equation}
...
\begin{proof}
	...
	\end{equation*}
	Or d'après la proposition 1.17, $\PR_{p_{i}}^{e_{i}}\bar{\PR_{i}}^{e_{i}} = p_{i}^{e_{i}}\OR_{\Delta}$, donc 
	\begin{equation*}
...
Dans le contexte de notre problème, on sait que l'idéal est principal par construction, et on connaît la forme quadratique $f$ associée. On cherche donc $(x,y)\in\Z^2$ solution de $f(x,y) = 1$. La notion de forme réduite va nous permettre de conclure. 
...
On en déduit l'algorithme $5$ permettant de trouver le générateur $\beta$ d'un idéal principal primitif.
...
0n commence par énoncé la complexité des algorithme $2$ et $5$:
...
\begin{Coro}
	Soit $(a,b,c)$ une forme quadratique binaire primitive de discriminant $\Delta$. La complexité de l'algorithme 2 appliqué à $(a,b,c)$ est en 
	\begin{equation*}
		O\left( \log(a)\log\left( \frac{a}{\sqrt{|\Delta|}} \right) +\log(b)\right).
	\end{equation*}
	En particulier si $b\leq 2a$ alors la complexité de l'algorithme 2 est majorée en $O(log(a)^2)$.
...
\begin{Prop}
	Soit $(1,a,b) = \beta\OR_{\Delta}$ un idéal principal primitif de $\OR_{\Delta}$ donné sous forme standard (en particulier $a$ est la norme de $\beta$). L'algorithme 8 trouve $\beta$ avec une complexité en
	\begin{equation*}
...
Lorsque l'on veut calculer une isogénie de degré $l$, \cite{Velusqrt} propose une méthode pour évaluer les polynômes intervenant dans les formules de Vélu dans $\F_{q}[X]$ en $O(\sqrt{l})$ opérations dans $\F_{q}$. Les résultats suivants en découle.
...
\begin{Rem}
	SquareVélu est une amélioration asymptotique de l'application naïve des formules de Vélu. Cependant pour les petits nombres premiers, l'application naïve reste plus rapide. En pratique avec l’implémentation de Sage, il est conseillé d'utiliser SquareVélu uniquement si $l > 100$. 
\end{Rem}
...
On en déduit l’algorithme 5, proposé dans \cite{BrokerCharlesLauter}[\S 3.2] comme méthode générale pour le calcul d'isogénie de degré $l$ en $O(l)$.

\begin{algorithm}
	\caption{Point générateur d'un groupe de $\LR$-torsion, méthode 1}
	\begin{algorithmic}[1]
		\Require Une courbe elliptique $E$ à multiplication complexe définie sur $\F_{q}$, d'anneau d'endomorphisme $\OR_{\Delta}$, de conducteur $f$ et de conducteur maximal $f_{m}$, un nombre $l$ premier vérifiant l'hypothèse 1, un idéal propre $\LR\subset\OR_{\Delta}$ sous la forme $\LR = \left[ l, \frac{c + d\pi_{q}}{f_{m}}\right]$
...
On remarque d'abord que, partant de la forme standard de l'idéal $\LR$, on peut toujours se ramener à $d = 1$ (il suffit d'expliciter les calculs de la proposition 4.25)\textbf{Si l'on retire la section 4 il faut détailler un peu ici}. Si $P$ est un générateur du groupe de torsion, alors $\pi_{q}(P) = -cP$ : $P$ est un vecteur propre de $\pi_{q}$ restreint à $E[l]$. On se ramène donc à une étude $\pi_{q}$ restreint à $E[l]$.
...
\textbf{ Cas de 0 isogénie quand l est ramifié ?}

\end{Prop}

 Puisque $l$ est supposé décomposé et premier avec $f_{m}$, on sait qu'il existe exactement $2$ isogénies définies sur $\F_{q}$. On note $v_{1}$ et $v_{2}$ les deux valeurs propres de $\pi_{q}$ restreint à $E[l]$. 
...
Puisque l'on connait $c$, on sait quelle valeur propre est associée à l'isogénie que l'on cherche. La proposition précédente nous donne alors le degré de l'extension dans laquelle on doit chercher $E[l]$. On note $r$ ce degré. On cherche alors $\K\in E(\F_{q^{r}})$ un vecteurs propre de $\pi_{q}$ pour la valeurs propres $-c$, d'ordre $l$. L'idée et que si la structure de groupe de $E(F_{q^{r}})$ est 
\begin{equation*}
...
On peut donc chercher toute les valeurs de $n_{1}$ possible, en utilisant la proposition précédente pour limiter le nombre de tests. On en déduit l'algorithme 6, qui améliore l'algorithme 5. 
...
\begin{enumerate}
	...
	\item Soit $h$ une racine de $\Psi_{l}(j,Y)$ dans $\F_{q}$, alors il est possible d'obtenir un modèle défini sur $\F_{q}$ de la courbe $E'$ de $j$-invariant $h$, à partir de la donnée de $E$, $h$ et $l$, en menant des calculs uniquement dans $\F_{q}$.
	...
	\item Soit $F_{h}$ le polynôme qui décrit le noyau de $\phi : E\rightarrow E'$. Soit $\tilde{F}_{h}$ défini de la même façon à partir de relèvements de During de $E$, $E'$ et $\phi$. Soit $\wp$, $\wp'$ les fonction de Weierstrass associée aux relèvements de $E$ et $E'$. Les coefficients des séries de Laurent de $\wp$ et $\wp'$ peuvent être calculer par récurrence.
	
	\item Il existe une série entière $S$, dont tout les coefficients dépendent de l'écriture en série de Laurent de $\wp$, $\wp'$ et du relèvement de $p_{1}$, telle que
	\begin{equation}
		z^{l-1}F'_{h}(\wp(z)) = \exp(S),
	\end{equation}
	
	\item On peut alors retrouver les coefficients de $F'_{h}$ en identifiants les $(l-1)/2$ premiers termes des deux membres de l'équation. Si chaque terme a une bonne réduction modulo l'idéal de réduction $\PR$ au dessus de $p$, alors on obtient directement les coefficient de $F_{h}$. La bonne réduction n'est cependant pas garanti si l'on doit calculer trop de termes : il faut que $l$ vérifie
	\begin{equation*}
		l < 4*(p+1).
	\end{equation*}
\end{enumerate}
...
\begin{Prop}
	...
	\end{equation*}
	Soit $(R_{1}(X), YR_{2}(X))$ les fonctions rationnelles définissant l'endomorphisme $[-c]$. Alors $F$ est le polynôme décrivant $E\left[ \LR\right]$ si et seulement si, dans le corps de fraction de $R$,
	\begin{equation*}
		\bar{X}^q = R_{1}(\bar{X})\;,\;\bar{Y}^{q} = \bar{Y}R_{2}(\bar{X}).
	\end{equation*}
\end{Prop}

On en déduit l'algorithme $7$ permettant de calculer le polynôme décrivant le noyau d'une isogénie, à partir d'un modèle de départ et d'un idéal associé. On suppose aussi que le polynôme modulaire $\Psi_{l}$ est pré-calculé.  

\begin{algorithm}
	\caption{Méthode d'Elkies}
	\begin{algorithmic}[1]
		\Require Une courbe elliptique $E$ à multiplication complexe définie sur $\F_{q}$, d'anneau d'endomorphisme $\OR_{\Delta}$, un nombre $l$ premier vérifiant l'hypothèse 1, un idéal propre $\LR\subset\OR_{\Delta}$ sous la forme $\LR = \left[ l, \frac{c + d\pi_{q}}{f_{m}}\right]$, le polynôme modulaire $\Psi_{l}$.
...
On se donne un modèle $E$, $l$ et $n$ vérifiant l'hypothèse 1. Soit $\LR$ un idéal de $End(E)$ de norme $l$. On est capables d'exprimer la classe $\CL$ comme un produit de classes représentées par des idéaux $\PR_{1}, \ldots,\PR_{N}$ de petites normes vérifiant tous l'hypothèse 1. On peut donc calculer des isogénies dont les noyaux sont des groupes de $\PR_{i}$-torsion, et ainsi reproduire l'action de $\CL$ sur $E$ en suivant sa factorisation. 
...
\end{equation*}
où $\alpha\OR_{\Delta}$ est un idéal principal fractionnaire. On sait calculer $\alpha = \beta/m$ avec $\beta = \frac{u + v\pi_{q}}{f_{m}}\in\OR_{\Delta}$. D'après l'hypothèse 1, les nombres premiers $p_{i}$ sont premiers avec $|E(\F_{q^{n}})|$, donc $m$ est aussi premier avec ce cardinal. On a de plus supposé que $f_{m}$ est premier avec $|E(\F_{q^{n}})|$ dans l'hypothèse 1, ce qui permet de calculer $k = (mf_{m})^{-1}$ modulo $|E(\F_{q^{n}})|$. L'endomorphisme $k(u + v\pi_{q})$ est donc bien défini. 
...
\begin{proof}
	Soit $\tilde{E}$, $\tilde{E_c}$, $\tilde{E'} = \widetilde{E/E\left[\LR\right] }$ trois courbe elliptique définie sur $\Qb$, obtenue par le théorème de relèvement de Deuring appliquée via l'endomorphisme de Frobenius, et via le même idéal de réduction $\PR$ au dessus de la caractéristique $p$. Soit $\Lambda$ un réseau de $\C$ tel que $E_{Lambda} = \tilde{E}$. Alors la factorisation de $\LR$ implique que
	\begin{equation*}
