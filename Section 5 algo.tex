...
\textbf{Déplacer cette remarque à l'algorithme 2}

On en déduit l'algorithme $8$ permettant de trouver le générateur $\beta$ d'un idéal principal primitif.
...
0n commence par énoncé la complexité des algorithme $2$ et $3$:
...
\cite{JaoSoukharev} et \cite{Coh00}[\S 5.5.1] propose une astuce pour optimiser la complexité des l'algorithme 3 et 4 : Si $N$ est trop petit, on risque de ne pas avoir une famille génératrice avec l'algorithme 3, et il est plus difficile de trouver par hasard une classe facilement factorisable lors de l'algorithme 4. Mais si $N$ est trop grand, l'algorithme $3$ prend plus de temps. Il sera aussi plus lent de vérifier si un entier se factorise dans la famille de nombres premiers $L_{premier}$. La borne théorique $N = 2\log(|\Delta|)^{2}$ étant supposé pessimiste \cite{Coh00}[\S 5.5.1], on choisit $N$ différemment :

\begin{Def}
	Soit $\Delta$ un discriminant. On défini la complexité sous exponentielle en $\log(|\Delta|)$ de la façon suivante. Pour $\alpha\in\left] 0,1 \right[$ et $z>0$, on note $L(\alpha,z)$ toutes expressions de la forme
	\begin{equation*}
...
\begin{Def}
	Soit $\Delta$ un discriminant et $z>0$ fixé que l'on calculera par la suite. On note $N_{t} = 2\log(|\Delta|)^{2}$ et $N_{p} = L(1/2,z)$. $N_{t}$ est appelée la borne théorique, et $N_{p}$ la borne pratique. 
\end{Def}

\begin{Rem}
	Même si la borne théorique est asymptotiquement plus petite que la borne pratique, pour les tailles cryptographiques c'est la borne sous exponentielle qui sera plus petite \cite{Coh00}[5.5.1]. Dans \cite{JaoSoukharev}, la borne pratique est cependant supposée (sans preuve) suffisamment grande pour que l'algorithme $3$ appliquée à $N_{p}$ retourne une famille génératrice.
\end{Rem}
...
\begin{Prop}
	Soit $t = C_{t}\log\left( |\Delta|\right) /\log\left( \log\left( |\Delta|\right) \right)$ le nombre de pas de la marche aléatoire de l'algorithme $4$. Soit $k$ la taille des listes retournées par l'algorithme $3$. Alors le nombre d'opération de chaque itération de la boucle while des lignes $5$ à $19$ de l'algorithme $4$ est en moyenne 
	\begin{equation*}
		O\left( \frac{\log(N)^2\log(|\Delta|)^3}{\log(\log(|\Delta|))} + \frac{N}{\log(N)}\log(|\Delta|)^{2}\right)
	\end{equation*}
\end{Prop}

\begin{proof}
	Durant la marche aléatoire, chaque indice est choisi en moyenne $t/k$ fois. Si $N \geq N_{t}$, d'après la proposition 5.31 on peut supposer qu'en moyenne chaque indice est incrémenté au plus une fois, et que la liste $L_{x}$ ne contient que des éléments nuls sauf $t$ valeurs égales à $\pm 1$. Sous cette hypothèse le calcul de $classe_{test}$ des lignes $12$ à $18$ demande $t$ produits, chacun entre une forme réduite et une forme première associée à un $p_{i} \leq N$, et à chaque fois suivi d'une réduction par l'algorithme $2$. Puisque les coefficients $a$, $b$ d'une forme réduite $(a,b,c)$ vérifie
	\begin{equation*}
		|b| \leq a \leq \sqrt{\frac{|\Delta|}{3}},
	\end{equation*} 
	...
	\end{equation*}
	Après avoir calculer $classe_{test}$, il faut tester s'il se factorise via le théorème 5.12. On note maintenant $classe_{test} = (a,b,c)$. Il s'agit de déterminer si $a$ se factorise sur les éléments de $L_{premier}$, autrement dit si $a$ est $L_{premier}$-friable. On applique ECM pour factoriser $a$. \textbf{TODO analyse complexité avec ECM}
	
	
	
	Ancienne preuve obsolète : On procède par essais erreur, (voir \cite{Bern04} pour des améliorations possibles), ce qui demande pour chaque $p_{i}$ au plus $O(\log(a))$ divisions euclidiennes, donc au plus $O(\log(a)^2)$ opérations, ceci $k$ fois. Comme $classe_{test} = (a,b,c)$ est réduite, la vérification de la ligne $5$ prend
	\begin{equation*}
		O(k\log(a)^2) = O\left( \frac{N}{\log(N)}\log(|\Delta|)^{2}\right) 
	\end{equation*}
\end{proof}

\begin{Prop}
	
	Soit $t = C_{t}\log\left( |\Delta|\right) /\log\left( \log\left( |\Delta|\right) \right)$ le nombre de pas de la marche aléatoire de l'algorithme $4$. Soit $k$ la taille des listes retournées par l'algorithme $3$. Si $N = N_{t}$, alors
	\begin{enumerate}
		\item Chaque itération de la boucle des lignes $5$ à $19$ de l'algorithme $4$ demande en moyenne $O(\log(|\Delta|)^4)$ opérations.
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item  Avec $N = N_{t}$, ceci (nb de calculs pour $classe_{test}$) devient
		\begin{equation*}
			O(t\log(N)^2\log(|\Delta|)^2) = O(\log(|\Delta|)^3\log(\log(|\Delta|))).
		\end{equation*}
		
		Avec $N = N_{t}$, ceci (factorisation du coef a) devient
		\begin{equation*}
			O(k\log(a)^2) = O\left( \frac{\log(|\Delta|)^4}{\log(\log(|\Delta|))}\right) = O(\log(|\Delta|)^{4}).
		\end{equation*}
		
		Ainsi lorsque $N = N_{t}$, l'étape ayant la complexité la plus lourde lors d'une itération de la boucle est la vérification de la ligne $5$, et la complexité d'une itération est en $O(\log(|\Delta|)^4)$.
		
		\item Avec $N = N_{t}$, Le nombre d'itérations de la boucle des lignes $5$ à $19$ est minoré d'après la proposition $5.32$ par
		\begin{equation*}
			O\left( \exp \left(  \log\log\left(|\Delta| \right)  \frac{\log\left(|\Delta| \right)}{4\log(\log(|\Delta|))} \right) \right) = O(\sqrt{|\Delta|})
		\end{equation*}
		ce qui implique un complexité au moins exponentielle en $\log(|\Delta|)$. On peut majorer le nombre d'itérations en considérant le cas où l'on ne cherche pas à factoriser, mais à directement trouver $a = 1$, c'est à dire arrêter la boucle lorsque $classe_{test}$ est la classe neutre du groupe $Cl(\OR_{\Delta})$. Comme le cardinal du groupe de classe est en $O(\sqrt{|\Delta|})$, en supposant les valeurs de $classe_{test}$ uniforme dans $Cl(\OR_{\Delta})$ grâce au choix de $t$, le nombre d'itérations est dans ce cas en $O(\sqrt{|\Delta|})$, donc exponentiel en $\log(|\Delta|)$. 
	\end{enumerate} 
	
\end{proof}

\begin{Prop}
	Soit $t = C_{t}\log\left( |\Delta|\right) /\log\left( \log\left( |\Delta|\right) \right)$ le nombre de pas de la marche aléatoire. Soit $k$ la taille des listes retournées par l'algorithme $3$. Si $N = N_{p}$, l'algorithme $4$ a une complexité sous-exponentielle en $\log(|\Delta|)$. Plus précisément si $N_{p} = L\left(\frac{1}{2}, z \right)$ alors l'algorithme $4$ a une complexité selon $\log(|\Delta|)$ en
	
	\begin{equation*}
		 O\left( L\left(\frac{1}{2}, z + \frac{1}{4z} \right) \right) 
	\end{equation*}
\end{Prop}

\begin{proof}
	Dans la preuve précédente, on a établit des formules de complexité en fonction de $N$. Lorsque $N = N_{p}$, toutes les étapes de l'algorithme dont la complexité dépend de $\log(N)$ sont polynomiales en $\log(|\Delta|)$. L'étape de vérification de la ligne $5$ admet une complexité en 
	\begin{equation*}
		O\left( \frac{N_{p}}{\log(N_{p})}\log(|\Delta|)\right) = O(N_{p})
	\end{equation*}
	La complexité de chaque itération de la boucle des ligne $5$ à $19$ est dominé par cette étape et est donc en $O(N_{p})$. Le nombre d'itérations est quand à lui de l'ordre de 
	\begin{equation*}
		O\left( \exp \left( \left( \log\log\left(|\Delta| \right) - \log\log\left( N_{p} \right)  \right)\frac{\log\left(|\Delta| \right)}{2\log\left( N_{p} \right)} \right) \right) = O\left( \exp \left( \log\log\left(|\Delta|  \right)\frac{\log\left(|\Delta| \right)}{2\log\left( N_{p} \right)} \right) \right)
	\end{equation*}
	On rappelle que $N_{p} = L(1/2,z)$ avec $z\in\R$ fixé à déterminer. Ainsi
	\begin{equation*}
		O\left( \exp \left( \log\log\left(|\Delta|  \right)\frac{\log\left(|\Delta| \right)}{2\log\left( N_{p} \right)} \right) \right) = O\left( L\left( \frac{1}{2},\frac{1}{4z}\right) \right) 
	\end{equation*}
	
	La complexité selon $\log(|\Delta|)$ de l'algorithme $4$ est donc en 
	\begin{equation*}
		O\left( N_{p}L\left( \frac{1}{2},\frac{1}{4z}\right)\right)  = O\left( L\left(\frac{1}{2}, z + \frac{1}{4z} \right) \right) 
	\end{equation*}
	
\end{proof}

L'algorithme $4$ influence fortement le temps de calcul de l'algorithme $9$. La seconde étape coûteuse est le calcul d'isogénie, principalement à cause du calcul et de la manipulation des polynômes modulaires. 
...
\begin{Prop}
	L'algorithme 7, appliqué à une courbe elliptique $E$ et un idéal $\PR_{i}\subset\OR_{\Delta}$ de norme $p_{i}$, retourne un polynôme $F$ décrivant $E[\PR_{i}]$ avec une complexité en 
	\begin{equation*}
...
\begin{proof}
	On se place dans le cadre de la proposition $5.31$ et on suppose en moyenne que chaque exposant non nul est égal à $1$. Pour tout exposant $e_{i}$ non nul, chaque calcul de composante $\phi_c$ demande de calculer et manipuler le polynôme modulaire $\Psi_{p_{i}}$. Dans l'analyse de complexité de \cite{JaoSoukharev}, la complexité citée du calcul de  $\Psi_{p_{i}}$ est en
	\begin{equation*}
		 O\left( p_{i}^{3 + \epsilon}\log\left( p_{i}\right)^{3 + \epsilon} \right)\;,\;\forall\epsilon >0.
	\end{equation*}
	Et chaque manipulation de $\Psi_{p_{i}}$ (dérivation partielle, évaluation) a un coup estimé par \cite{JaoSoukharev} en 
	\begin{equation*}
		O\left(p_{i}^{2 + \epsilon}\log\left( q \right)^{1 + \epsilon} \right)\;,\;\forall\epsilon >0.
	\end{equation*}
	En majorant $p_{i}$ par $N$, et d'après la proposition $5.31$, si l'on suppose que la manipulation de $\Psi_{p_{i}}$ domine la complexité du calcul de $\phi_c$, alors celle-ci est en
	\begin{equation*}
		O\left( t\left(N^3\log(N)^{2} + N^2\log(q) \right) \right). 
	\end{equation*}
\end{proof}

Le calcul de l'image du point $P$ demande de l'arithmétique dans $\F_{q^{n}}$, ce qui devient coûteux selon le paramètre $n$. La complexité va aussi dépendre des degrés des isogénies intermédiaires, donc de $N$.  
...
\begin{proof}
	Contrairement au reste de l'algorithme, ici les calculs se font dans le corps $\F_{q^{n}}$ car $P\in E(\F_{q^{n}})$. L'évaluation d'une isogénie de degré $p_{i}$ obtenue avec les formules de Vélu et l'algorithme BMSS est d'après \cite{JaoSoukharev} de $O(p_{i}^{2})$ opérations dans $\F_{q^{n}}$. Chacune demande $O(\log(q^n))$ opérations élémentaires. D'après la proposition $5.31$, on effectue $t$ évaluations de ce type. En majorant $p_{i}$ par $N$, on obtient que la complexité des répétitions de la ligne $14$ est en $O(tN^2\log(q^n))$. L’évaluation à la ligne $25$ est un calcul d'une isogénie scalaire explicite, ce qui est négligeable devant ce qui précède.  
\end{proof}

Le calcul d'un générateur de l'idéal principal dépend de la norme de l'idéal. La proposition $5.31$ va assurer que la norme de $\LR_{principal}$ est petite et que lui trouver un générateur n'est pas une étape coûteuse. 
...
\begin{proof}
	Le nombre de calcul de cette algorithme est le double du nombre de calcul nécessaire à la réduction de la forme quadratique associée à $\beta\OR_{\Delta}$. D'après l'analyse de l'algorithme $2$, on déduit la complexité de l'algorithme $8$. 
\end{proof}
...
\begin{Prop}
	La norme de l'idéal $\LR_{principal}$ calculé lors de la boucle des lignes $7$ à $17$ de l'algorithme $9$ est en $O(lN^t)$. Avec le choix de $t$ de la proposition $5.31$, le calcul de $\LR_{principal}$ demande une complexité en $O\left( \left( log(l) + tlog(N)\right) \right)$. La complexité de la recherche de $\beta$ à la ligne $18$ de l'algorithme $9$ est en $O\left( \left( log(l) + tlog(N)\right)^{2} \right)$.
\end{Prop}

\begin{proof}
	Ces résultats découlent de la proposition $5.31$. En majorant chaque norme de idéaux premier $\PR_{i}$ intervenant dans le calcul par $N$, d'après la proposition $5.31$ et la multiplicativité de la norme, $N\left(\LR_{principal} \right) \leq lN^t$. La complexité du calcul du produit de deux idéaux entier $\IR$ et $\LR$ donné sous forme standard est en $O\left( \max\left( N(\IR),N(\LR)\right) \right)$ ( voir \cite{JaoSoukharev}), donc la complexité du calcul de $\LR_{principal}$ peut être majorée en $O\left( \log\left(N\left(\LR_{principal} \right)  \right) \right)$. Si $(1,a,b) = \beta\OR_{\Delta}$, $b$ est obtenu par produit successif d'idéaux sous forme standard donc $b\leq 2a$ et la complexité de l'algorithme $8$ appliqué à $\LR_{principal}$ est majorée en $O\left( \log\left(a  \right)^2 \right) = O\left( \log\left(N\left(\LR_{principal} \right)  \right)^2 \right)$.
\end{proof}

On déduit de ce qui précède la complexité de l'algorithme $9$. 

\begin{Prop}
	Si $N = N_{t}$, l'algorithme $9$ admet une complexité en $O(\log(l))$, $O(log(q))$, $O(n)$ et $O(\sqrt{|\Delta|})$
\end{Prop}

\begin{proof}
	Il s'agit de remarquer pour chaque paramètre quelle étape domine la complexité. Les seules étapes dépendant de $l$ sont en $O(\log(l))$. La dépendance en $q$ est en $(log(q^n))$ pour le calcul de l'image du point $P$, d'où une complexité en $O(log(q))$ et en $O(n)$. La dépendance en $\Delta$ est dominé par la recherche de relation dans l'algorithme $4$, qui est en $O(\sqrt{|\Delta|})$. 
\end{proof}

\begin{Prop}
	Si $N = N_{p}$, l'algorithme $9$ admet une complexité en $O(\log(l))$, $O(log(q))$, $O(n)$ et 
	\begin{equation*}
		O\left( L\left(\frac{1}{2}, z + \frac{1}{4z} \right) + L\left(\frac{1}{2}, 3z \right) \right). 
	\end{equation*}
	 Le temps de calcul de l'algorithme est minimal lorsque $z = \frac{1}{2\sqrt{2}}$ et dans ce cas la complexité selon $\Delta$ est en \begin{equation*}
	 	O\left( L\left(\frac{1}{2}, \frac{3}{2\sqrt{2}} \right) \right)
	 \end{equation*}
\end{Prop}
	
\begin{proof}
	 Seule la dépendance en $\Delta$ change comparé à la proposition précédente. Les étapes sous-exponentielle en $\log(|\Delta|)$ sont
	 \begin{enumerate}
	 	\item L'algorithme $3$, en $O(L(\frac{1}{2}, z))$,
	 	\item L'algorithme $4$, en $O(L(\frac{1}{2}, z + \frac{1}{4z}))$,
	 	\item Le calcul de $L_{composantes}$, en $O\left( t\left(N^3\log(N)^{2} + N^2\log(q) \right) \right) = O(L(\frac{1}{2}, 3z))$.
	 	\item Le calcul de l'image du point $P$, en $O(tN^2\log(q^n)) = O(L(\frac{1}{2}, 2z))$
	 \end{enumerate}
	 Pour tout $z$, le calcul de $L_{composantes}$ domine en complexité à la fois l'algorithme $3$ et le calcul de l'image de $P$. Ainsi la complexité de l'algorithme $9$ est en
	 \begin{equation*}
	 	O\left( L\left(\frac{1}{2}, z + \frac{1}{4z} \right) + L\left(\frac{1}{2}, 3z \right) \right). 
	 \end{equation*}
	 On choisit $z > 0$ minimisant cette complexité : on veut équilibré les deux termes, c'est à dire choisir $z$ tel que
	 \begin{equation*}
	 	z + \frac{1}{4z} = 3z \iff z^2 = \frac{1}{8}
	 \end{equation*}
	 On choisit donc $z = \frac{1}{2\sqrt{2}}$. la complexité obtenue est alors en
	 \begin{equation*}
	 	O\left( L\left(\frac{1}{2}, \frac{3}{2\sqrt{2}} \right) \right)
	 \end{equation*}
