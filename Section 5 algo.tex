...
\begin{Prop}
	Soit $N$ un entier positif, $\Delta$ un discriminant, et $A = pf_{m}|E(\F_{q^{n}})|$. L'algorithme $3$ renvoie une famille supposée génératrice du groupe de classe de représentants de norme premier bornée par $N$ avec une complexité majorée par 
	\begin{equation*}
		O(N\log(N)^4).
	\end{equation*} 
\end{Prop}
...
Les deux propositions suivantes vont permettre d'analysé la complexité de l'algorithme $4$ et des algorithmes suivant.

\begin{Prop}
	Soit $t = C_{t}\log\left( |\Delta|\right) /\log\left( \log\left( |\Delta|\right) \right)$ le nombre de pas de la marche aléatoire de l'algorithme $4$. Soit $k$ la taille des listes retournées par l'algorithme $3$. Soit $x_{i}$, $0\leq i \leq k-1$ les éléments de $L_{x}$ obtenues avec les lignes $7$ à $11$ de l'algorithme $4$. Alors les $|x_{i}|$ sont des variables aléatoires d'espérance majorée par $O\left( \frac{1}{\log(|\Delta|)}\right)$. 
\end{Prop}
