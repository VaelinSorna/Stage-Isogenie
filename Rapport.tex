...
\begin{Prop}\cite{Jao_2009}[Lem 2.1]
	...
	Soit $G = Cl(\OR_{\Delta})$ et $S = L_{ideaux}\cup (L_{ideaux})^{-1}$. On considère une marche aléatoire en $t$ étape partant d'un sommet quelconque. Si 
	\begin{equation*}
		t = O(\frac{\log(|\Delta|)}{\log(\log(|\Delta|))})
	\end{equation*} 
	Alors pour toute partie $S_{0}\subset G$, la marche aléatoire termine dans $S_{0}$ avec probabilité au moins $\frac{|S_{0}|}{2|G|}$. En particulier la probabilité de terminer dans $S$ est au moins $\frac{|S|}{2|G|}$.
\end{Prop}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Require Un entier positif $N$, un discriminant $\Delta$, la caractéristique $p$, le conducteur maximal $f_{m}$, le cardinal $|E(\F_{q^{n}})|$. Un idéal $\LR\subset\OR_{\Delta}$. 
		...
		\For{$0<i<C\log\left( |\Delta|\right) /\log\left( \log\left( |\Delta|\right) \right) $}
		...
		\EndFor
		\State $ideal_{test}\gets\LR*\PR_{1}^{x_{1}}\PR_{2}^{x_{2}}\ldots\PR_{k}^{x_{k}}$
...
\begin{Prop}
	Soit $t = C\log\left( |\Delta|\right) /\log\left( \log\left( |\Delta|\right) \right)$ le nombre de pas de la marche aléatoire. Soit $k$ le nombre d'indice dans les listes retournées par l'algorithme $3$. Si $N = 2log(|\Delta|)^2$ et $t\leq k$, alors l'algorithme $4$ a une complexité moyenne en 
\end{Prop}

\begin{proof}
	 D'après le théorème des nombres premiers, $k = O(N/log(N))$. En particulier $t/k = O(1)$ et de l'ordre d'une constante $C_{0}$. Si $t\leq k$, $C_{0} \leq 1$. Durant la marche aléatoire, chaque indice est choisi en moyenne $t/k$ fois, donc moins d'une fois. On peut donc estimer en moyenne qu'à l'étape $6$ la liste $L_{x}$ ne contient que des $0$ et des $1$, et au plus $t$ $1$. $ideal_{test}$ est donc de norme $\leq lp_{k-t}\ldots p_{k} \leq lp_{k}^{t}$. Calculer sa forme réduite $classe_{test}$ se fait en $O(log(lp_{k}^{t})) = O(log(l) + tlog(N))$. Vérifié si $classe_{test}$ se factorise demande $O(k)$ test de divisibilité et division euclidienne, chacune sur un coefficient $a$ d'une forme quadratique réduite : $a < \Delta$. Au total les étapes $11$ et $12$ demandent $O(klog(|\Delta|))$ opérations dans $\Z$. Les étapes $13$ à $20$ sont élémentaires. Il reste à savoir combien d'essais pour $L_{x}$ doit être fait en moyenne avant de réussir le test de l'étape $11$. Par choix de $t$, et avec les notations de la proposition 5.14, il faudra en moyenne $O(\frac{|G|}{|S|})$ essais. Le nombre de classes $|G|$ est de l'ordre de $O(\sqrt(|\Delta|))$. \textbf{Estimer le cardinal de $S$ ?} 
\end{proof}

\cite{JaoSoukharev} propose une complexité différente : On peut chercher à l'optimiser par rapport à l'implémentation de l'algorithme final au complet.
...
\begin{Prop}
	L'algorithme $7$ admet une complexité en $O(log(ql)M(l))$
\end{Prop}
...
L'article \cite{BrokerCharlesLauter} annonce une complexité en $O(log(l))$, $O(log(q))$, $O(n)$ et $O(|\Delta|)$. Cependant ils font une analyse au pire cas de l'algorithme $4$. L'article \cite{JaoSoukharev} décrit une amélioration sous-exponentielle en $|\Delta|$ en menant une étude par analyse-synthèse de l'algorithme $9$. On regroupe les résultats énoncé précédemment pour énoncer la complexité suivante :
