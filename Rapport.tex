...
\begin{center}
	\textbf{Dans toutes la suite, on suppose que $l$ ne divise pas $p*f_{m}*|E(\F_{q^{n}})|$} 
\end{center}

Cette condition nous oblige à éviter certains nombres premiers. Elle a cependant plusieurs avantages :
\begin{itemize}
	\item Sous cette condition, $l$ ne divise pas $f_{m}$ donc toutes les $l$-isogénies séparables sont horizontales. En particulier leurs noyaux sont accessibles comme groupes de torsions d'idéaux de norme $l$.
	...
	\item Si $l$ ne divise pas $p*f_{m}*|E(\F_{q^{n}})|$, $f_{m}$ et $|E(\F_{q^{n}})|$ sont inversibles modulo $l$. Le fait que $f_{m}$ soit inversible sera utile par la suite pour manipuler des endomorphisme de la forme $\left[ \frac{c + d\pi_{q}}{f_{m}}\right] $. Le fait que $|E(\F_{q^{n}})|$ soit inversible sera utile pour normaliser l'isogénie obtenue (Voir section 5.5, 5.6).\\
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Require Un modèle défini sur $\F_{q}$ de courbe elliptique $E$ à multiplication complexe, le discriminant $\Delta$ de son anneaux d'endomorphisme, $l$ premier ne divisant pas $p*f_{m}*|E(\F_{q^{n}})|$ tel que $\left( \frac{\Delta}{l}\right) = 1$
...
\begin{Prop}
	L'algorithme 7 appliqué à un idéal $\LR$ admet une complexité en $O(\log(N(\LR)^2))$.
\end{Prop}

\begin{proof}
	Le nombre de calcul de cette algorithme et le double du nombre de calcul nécessaire à la réduction de la forme quadratique $f$. D'après l'analyse de l'algorithme $2$, on déduit la complexité de l'algorithme $7$. 
\end{proof}
...
On se donne un modèle $E$ de corbe elliptique à multiplication complexe définie sur $\F_{q}$, et $\LR$ un idéal de norme $l$ vérifiant toutes les conditions voulues. On est capables de factoriser sa classe $\CL$ en un produit de classe représentée par des idéaux de petites normes $\PR_{1}, \ldots,\PR_{N}$ de petites normes vérifiant toutes les conditions voulues. On peut calculer des isogénies dont les noyaux sont des groupes de $\PR_{i}$-torsion, et ainsi reproduire l'action de $\CL$ sur $E$ en suivant sa factorisation. 
...
$E_{c}$ est un modèle de $E/E[\LR]$. Cependant la $l$-isogénie $\phi : E \rightarrow E/E[\LR]$ n'est pas égale à $\phi_c$. En effet $\phi_c$ est de degré $m =p_{1}^{e_{1}}\ldots p_{N}^{e_{N}}$ par multiplicativité du degré. On a cependant la relation
\begin{equation*}
	\LR = (\alpha\OR_{\Delta})\PR_{p_{1}}^{e_{1}}\PR_{p_{2}}^{e_{2}}\ldots\PR_{p_{N}}^{e_{N}}
\end{equation*}
où $\alpha\OR_{\Delta}$ est un idéal principal fractionnaire. On est capable de calculer $\alpha$, mais puisque l'idéal principal n'est pas entier on ne peut pas lui associer un endomorphisme. Cependant on sait calculer $\alpha = \beta/m$ avec $\beta = \frac{u + v\pi_{q}}{f_{m}}\in\OR_{\Delta}$. On a choisi les $p_{i}$ premier avec $|E(\F_{q^{n}})|$, donc $m$ est aussi premier avec ce cardinal. \textbf{On a de plus supposé que $f_{m}$ est premier avec $|E(\F_{q^{n}})|$} ce qui permet de calculer $k = (mf_{m})^{-1}$ modulo $|E(\F_{q^{n}})|$. L'endomorphisme $k(u + v\pi_{q})$ est donc bien défini. 

\begin{Prop}
	Avec les notations précédentes, 
	\begin{equation*}
		\phi = (k(u + v\pi_{q}))\circ\phi_c
	\end{equation*}
\end{Prop}

\begin{proof}
	On va montrer que deux modèles explicites de ces isogénies partant de $E$ on le même noyaux. Le noyau de $\phi$ est $E\left[ \LR\right] $. Celui de $\phi_c$ est $E\left[\PR_{1}^{e_{1}} \ldots\PR_{N}^{e_{N}}\right]$. \textbf{A FINIR}
\end{proof}

On se résume ce qui précède en énonçant l'algorithme 8 :
