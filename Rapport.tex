...
Dans sa thèse, Kohel décrit la structure des graphes de $l$-isogénies, avec $l$ premier, comme des "volcans d'isogénies". Nous allons cités les résultats sur cette structure, en les admettant. 
...
\begin{Prop}
	Avec les notations précédentes, si $\phi$ est de degré $l$ premier, alors :
	\begin{enumerate}
...
\begin{Prop}
	...
	Avec les notations précédentes, soir $r = f_{m}/f$. On distingues les cas suivants :
	
	D'abord selon si $l$ est un nombre premier divisant $f$ :
	
	\begin{enumerate}
		\item Si $l$ ne divise pas $f$, alors il existe exactement $1 + \left( \frac{\Delta}{l}\right)$ $l$-isogénies horizontales définies sur $\F_{q}$.
...
\begin{Rem}
	Pour tout nombre premier $l$ ne divisant pas $f_{m}$, il existe $1 + \left( \frac{\Delta}{l}\right)$ $l$-isogénies horizontales, c'est à dire $0$, $1$ ou $2$, et aucune ascendante ni descendante.
...
On s'intéresse d'abord à une procédure naïve à partir de la formule de Vélu est la suivante :
...
Dans la section 7.2 on étudie comment appliquer efficacement la formule de Vélu, et la complexité obtenue. Ensuite on s'intéresse en 7.3 au calcul d'un générateur de $E\left[ \LR\right]$.

On obtiendra une complexité en $l$ insatisfaisante. L'algorithme de Broker, Charle et Lauter consiste à améliorer cette procédure en se ramenant à des idéaux de petites normes : l'idée est d'engendré $Cl(\OR_{\Delta})$ par des classes représenté par des idéaux de petites normes, puis factoriser $\CL$ dans cette famille génératrice. 

On étudie cela dans la section 7.4. On obtiendra $\LR$ sous forme d'un produit de petit idéaux, mais à un idéal principal prêt. Il faudra trouver un générateur d'un idéal, ce que l'on étudiera en section 7.5.
