...
\begin{enumerate}
	...
	\item On voudra appliquer calculer des isogénie associée aux idéaux représentant les générateurs de $\CL$, il faut dont qu'il vérifie aussi les conditions fixées sur $l$ : On veut pouvoir choisir des représentants de normes premier bien choisies. 
\end{enumerate}
...
La forme réduite sera adapté pour faire des calculs dans le groupe de classe. On voudra cependant par la suite se ramener à des idéaux toujours premier avec le conducteur maximal, ce qui n'est pas garanti pour l'idéal réduit.
...
\begin{Rem}
	...
	L'intérêt des représentations de Seysen est qu'il est plus utile de connaître une racine modulo $4p$ que modulo $p$, comme nous le verrons plus tard.
\end{Rem}
...
En pratique, Seysen cite deux résultats proposant $C= 280$ ou même $C = 2$.

\begin{Rem}
	Dans l'article de Jao, Soukharev, une borne sous exponentielle est proposé par analyse-synthèse, et n'admet pas l’hypothèse de Riemann généralisée. Nous reverrons cela dans la section 7.7.
\end{Rem}

On peut donc appliquer la procédure naïve. En pratique on pose $N = 2\times \log(\Delta)^{2}$ en supposant $C = 2$ n'est pas optimal et que la marge d'erreur dû aux premiers ignorés ne pose pas de problème. 
...
Une idée naïve consiste à chercher une factorisation au hasard. Deux faits vont permettre de rendre cette idée effective :

\begin{enumerate}
	\item Certaines classes représentés par des formes réduites aux bonnes propriétés vont être explicitement et rapidement factorisable grâce à la convention de Seysen.
	\item Les marches aléatoires dans le groupe de classe vont avoir de bonnes probabilités de terminer en un temps sous-exponentiel.
\end{enumerate}
...
Les bonnes propriétés des marches aléatoires sur les groupes de classe rendent cette procédure envisageable. Sans rappler la théorie des graphes expanseurs (voir cours Biasse), on cite le résultat principal :
...
Concrètement, dès que l'on enchaîne des marches aléatoires sont de l'ordre de $\frac{\log(|\Delta|)}{\log(\log(|\Delta|))}$, les résultats seront uniformément répartis parmi les éléments de $Cl(\Delta)$
...
\begin{Prop}
	...
	Si l'on connaît tout les $s$ tel que $0 < s < 2a$ et $s^2 = \Delta [4a]$, l'algorithme de Cornacchia fourni la solution à l'équation
	
\end{Prop}

Détailler cornacchia, lien avec $b$, notion de "bonne racine", et stabilité des étape de cornacchia dans $(1,a,b)$ ?
