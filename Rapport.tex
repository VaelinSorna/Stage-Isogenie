...
\begin{Def}
	Soit $\phi : E\rightarrow E'$ une isogénie sous forme explicite, $\omega_{E}$ et $\omega_{E'}$ les invariants différentiels associés respectivement aux modèles de départ et d'arrivé. On dira que cette forme explicite de $\phi$ est normalisée si $\phi^{*}(w_E') = w_E$
...
\section{Courbes à multiplication complexe sur corps finis}



 Étant donnée une courbe elliptique $E$ définie sur un corps fini $\F_{q}$, on souhaite calculer sous réserve d'existence une isogénie de degré $l$ donné, définie sur $\F_{q}$, partant de $E$. Pour cela on fait le lien entre $E$ et une courbe elliptique définie sur $\C$ à multiplication complexe de même anneaux d'endomorphisme. On peut alors expliciter le noyau de l'isogénie cherchée comme groupe de torsion d'un certain idéal entier $\LR$. On énoncera de plus un résultat dû à Kohel : la structure de volcan d'isogénie. Cela fournira un critère simple sur $l$ pour assurer l'existence d'une isogénie de degré $l$ définie sur $\F_{q}$.  
...
Pour tout ordre quadratique imaginaire $\OR_{\Delta}$, On souhaite comparer $Ell_{\C}(\OR_{\Delta})$ et $Ell_{\F_{q}}(\OR_{\Delta})$. Plus précisément, on cherche les corps finis $\F_{q}$ tels que $Ell_{\C}(\OR_{\Delta})\simeq Ell_{\F_{q}}(\OR_{\Delta})$. On commence par réduire l'étude de $Ell_{\C}(\OR_{\Delta})$ à un sous-corps de $\C$.
...
Chaque élément de $Ell_{\C}(\OR_{\Delta})$ peut être décrit par son $j$-invariant.
...
On n'aura pas besoin de se placer sur le corps $\Q$ tout entier, mais seulement sur le corps engendré par les $j$-invariants de $Ell_{\Qb}(\OR_{\Delta})$. En effet il suffit de se placer sur un corps qui garanti l'existence d'un modèle de chaque courbe.
...
Soit $E$ une courbe elliptique définie sur $\C$. D'après ce qui précède, il existe un corps de nombre $\HR$ sur lequel $E$ est défini. Quitte à prendre l'extension galoisienne de $\HR$, on pourra supposer que $\HR$ est galoisien sur $\Q$ si besoin. Soit $p$ un nombre premier. On va construire une courbe elliptique $\tilde{E}$ définie sur un corps fini $\F_{q}$ de caractéristique $p$.

\begin{Def}
	Soit $\HR$ un corps de nombre, $p$ un nombre premier. Soit $\PR\subset\OR_{\HR}$ un idéal premier au dessus de $p$. On note $A_{\PR} = (\OR_{\HR})_{(\PR)}$ le localisé de $\OR_{\HR}$ en $\PR$. 
\end{Def}

\begin{Def}
	Soit $E$ une courbe elliptique définie sur $\HR$. Soit $p$ un nombre premier, $\PR$ un idéal de premier de $\OR_{\HR}$ au dessus de $p$. $E$ admet une bonne réduction modulo $\PR$ si elle admet un modèle $E_{0}$ à coefficient dans $A_{\PR}$ dont le discriminant n'est pas un élément de $\PR$. Ce modèle n'est pas nécessairement sous forme d'équation de Weierstrass réduite. On note $\tilde{E}$ la courbe algébrique définie par le modèle $E_{0}$ modulo $\PR$. $\tilde{E}$ ne dépend pas du choix de modèle $E_{0}$. 

\end{Def}

\begin{Prop}
	Soit $E$ une courbe elliptique définie sur un corps de nombre $\HR$. Soit $p$ un nombre premier, $\PR$ un idéal de premier de $\OR_{\HR}$ au dessus de $p$, et $f_{p}$ le degré d'inertie de $p$ relativement à $\PR$. Si $E$ admet une bonne réduction modulo $\PR$, alors la réduction $\tilde{E}$ est une courbe elliptique définie sur $\F_{q}$ où $q = p^{f_{p}}$
\end{Prop}

\begin{proof}
	Par définition, $\tilde{E}$ est définie par une équation de Weierstrass à coefficient dans $A_{\PR}/\PR$. Or l'application
	\begin{equation*}
		\Psi : \widetilde{\left( \frac{a}{b}\right)}\in A_{\PR}/\PR\longmapsto \tilde{a}\tilde{b}^{-1}\in\OR_{\HR}/\PR\simeq\F_{q}
	\end{equation*}
	Et une bijection d'inverse
	\begin{equation*}
		\Psi' : \tilde{a}\in\OR_{\HR}/\PR\longmapsto \widetilde{\left( \frac{a}{1}\right)}\in A_{\PR}/\PR
	\end{equation*}
	Donc $\tilde{E}$ est définie par une équation de Weierstrass à coefficient dans $\F_{q}$. Comme le discriminant de $E$ n'est pas dans $\PR$ par hypothèse de bonne réduction, $\tilde{E}$ est une courbe algébrique lisse, donc une courbe elliptique.
\end{proof}

\begin{Rem}
	Lorsque $\HR$ est galoisien, le cardinal du corps finis sur lequel on défini la réduction de $E$ ne dépend pas de $\PR$. 
\end{Rem}

Lorsque deux courbes $E$ et $E'$ définies sur un corps de nombre $\HR$ admette une bonne réduction modulo $\PR$ au dessus de $p$, on peut définir la réduction d'une isogénie $\phi : E\rightarrow E'$ définie sur $\HR$.

\begin{Def}
	Soit $E$ et $E'$ deux courbes elliptiques définies sur un corps de nombre $\HR$, qui admettent une bonne réduction modulo $\PR$ au dessus de $p$. Soit $\phi : E\rightarrow E'$ une isogénie définie sur $\HR$ Alors il existe une forme explicite de $\phi$ à coefficient dans $A_{\PR}$. La réduction de $\phi$ modulo $\PR$, notée $\tilde{\phi}$, est définie en considérant cette forme explicite modulo $\PR$. On dit que $\phi$ admet une bonne réduction modulo $\PR$ si $\tilde{\phi}$ est non dégénérée.  
\end{Def}

\begin{Rem}
	
	On peut réduire des isogénie $\phi : E\rightarrow E'$ qui ne sont pas définie sur $\HR$. En effet il existe une extension finie $\Lc$ de $\HR$ sur laquelle $\phi$ est définie. Quand on réduit $E$ et $E'$ modulo $\PR$, on réduit $\phi$ modulo $\PR_{\Lc}$ un idéal premier de $\OR_{\Lc}$ au dessus de $\PR$. On peut alors considérer les réduction de $E$ et $E'$ modulo $\PR_{\Lc}$, on trouvera les mêmes réductions que celles modulo $\PR$
\end{Rem}

La réduction des isogénie est alors une application injective :

\begin{Prop}\cite{Sil94}[II, 4.4]
	
	Soit $E$, $E'$ deux courbes elliptiques définies sur un corps de nombre $\HR$, qui admettent une bonne réduction modulo $\PR$ au dessus de $p$. Soit $Hom_{\HR}(E, E')$ l'ensemble des isogénies définies sur $\HR$ entre $E$ et $E'$. L'application 
	\begin{equation*}
		Hom_{\HR}(E, E')\rightarrow Hom_{\HR}(\tilde{E}, \tilde{E'}),\;\phi\mapsto\tilde{\phi}
	\end{equation*}
	est injective, et préserve le degré : $deg(\phi) = deg(\tilde{\phi})$
	
\end{Prop}

On cherche à savoir sous quels condition on obtient un isomorphisme $End(E)\simeq End(\tilde{E})$. On cite pour cela un critère dû à Deuring :

\begin{The}\cite{Lang}[13.12]
	
	Soit $E$ une courbe elliptique à multiplication complexe définie sur un corps de nombre galoisien $\HR$, avec $End(E)$ de conducteur $f$. Soit $\K$ le corps quadratique imaginaire dont $End(E)$ est un ordre. On note $f = p^r f_{0}$ tel que $p$ ne divise pas $f_{0}$. Soit $\PR$ un idéal premier de $\OR_{\HR}$ au dessus de $p$.
	\begin{enumerate}
...
Si on se donne une courbe elliptique $E$ définie sur $\C$ à multiplication complexe, on choisira $p$ premier avec $f$ et décomposé dans le corps quadratique imaginaire $\K$. On pourra alors réduire $E$ sur $\F_{q}$ où $q$ dépend du choix d'un corps de nombre $\HR$ et d'un idéal de réduction $\PR$.

\subsection{Théorème de relèvement de Deuring}

On se donne désormais une courbe elliptique $E'$ définie sur un corps fini $\F_{q}$, à multiplication complexe. On souhaite voir $E'$ comme la réduction d'une certaine courbe $E$ définie sur un corps de nombre $\HR$, par un certain idéal $\PR$. Il faut d'abord que le critère du théorème 4.9 soit respecté, ce qui est toujours le cas

\begin{The}\cite{Lang}[13.5]
	
	Soit $E'$ une courbe elliptique définie sur un corps fini $\F_{q}$ de caractéristique $p$. Si $E'$ est à multiplication complexe, soit $f$ le conducteur de $End(E')$ et $\K$ son corps quadratique imaginaire. Alors $p$ est premier avec $f$ et décomposé dans $\K$. 
...
\begin{The}{Théorème de relèvement de Deuring}\cite{Lang}[13.14]
	...
	Soit $E'$ une courbe elliptique défini sur un corps fini $\F_{q}$, et $\alpha'$ un endomorphisme de $E'$ qui n'est pas une multiplication scalaire. Il existe une courbe $E$ définie sur un corps de nombre $\HR$, munie d'un endomorphisme $\alpha$ et d'un idéal premier $\PR$ au dessus de $p$, tels que la réduction $\tilde{E}$ modulo $\PR$ soit définie sur $\F_{q}$, et $\tilde{E}\simeq E'$ avec $\tilde{\alpha} = \alpha'$.
\end{The}
...
A REECRIRE JUSQUE ICI

Soit $E$ une courbe elliptique définie sur un corps fini $\F_{q}$. Alors le morphisme de Frobenius $\pi_{q}$ est un endomorphisme de $E$, inséparable donc non entier, et $End(E)$ contient $\Z[\pi_{q}]$. Ces deux ensembles sont des ordres du même corps quadratiques imaginaires $\K$. 
\begin{equation*}
	\Z[\pi_{q}]\subset End(E)\simeq\OR_{\Delta}\subset\OR_{\K}
\end{equation*} 

\begin{Def}
	Soit $E$ une courbe elliptique définie sur un corps fini $\F_{q}$. On note $f_{m}$ le conducteur de l'ordre $\Z[\pi_{q}]$, que l'on appel le conducteur maximal. On notera $f$ le conducteur de $End(E)\simeq\OR_{\Delta}$, ou $f_{E}$ si besoin. On appellera $f$ le conducteur de $E$, et $f_{m}$ son conducteur maximal. 
\end{Def}
...
\begin{Prop}
	Soit $\alpha\in\C$ une racine du polynôme caractéristique de $E$. Alors $Z[\pi_{q}] \simeq Z[\alpha] \subset \K$. Avec les notations de la proposition 1.2, soit $\OR_{\K} = \left[ 1, w_{\K}\right] $, de sorte que $\OR_{\Delta} = \left[ 1, fw_{\K}\right] $ et $Z[\alpha] = \left[ 1, f_{m}w_{\K}\right] $. Alors il existe un entier $s_{\alpha}$ tel que $\pm\alpha = f_{m}w_{\K} + s_{alpha}$
	
\end{Prop}

\begin{proof}
	$\alpha$ est racine de $X^{2} - tX + q$ de discriminant $\Delta_{m} = t^{2} - 4q = f_{m}^{2}d_{\K}$. Donc $\alpha = \frac{t\pm f_{m}\sqrt{d_{\K}}}{2}$, et $w_{\K} = \frac{d_{\K} + \sqrt{d_{\K}}}{2}$. On en déduis que  $\pm\alpha = f_{m}d_{\K} - \frac{f_{m}d_{\K} \pm t}{2}$ selon le choix de $\alpha$. Or $t^{2} - f_{m}^{2}d_{\K} = 4q$ est pair, donc $t$ et $f_{m}d_{\K}$ ont même parité et $s = -\frac{f_{m}d_{\K} \pm t}{2}$ est un entier.
	
\end{proof}

\begin{Rem}
	Avec $\alpha = \frac{t + f_{m}\sqrt{d_{\K}}}{2}$, on obtient un nombre complexe de même polynôme minimal annulateur que l'endomorphisme $\pi_{q}$, et tel que $\alpha$ et $f_{m}w_{\K}$ soient égaux à un entier prêt. Avec $\alpha' = \frac{t - f_{m}\sqrt{d_{\K}}}{2}$, $\alpha - f_{m}w_{\K}$ n'est pas un nombre réel. 
\end{Rem}

Dans toutes la suite, on associe à $\pi_{q}$ le nombre complexe $\alpha = \frac{t + f_{m}\sqrt{d_{\K}}}{2}$. Par abus de notation, on écrira parfois $\pi_{q} = \frac{t + f_{m}\sqrt{d_{\K}}}{2}$.
...
\begin{proof}
	Ces résultats découles de l'égalité $w_{\K} = \frac{\pi_{q} - s}{f_{m}}$ et des propositions 1.3 et 1.6.
\end{proof}
...
Pour normaliser une courbe elliptique $E$ définie sur $\F_{q}$ donnée, on procède ainsi : 
