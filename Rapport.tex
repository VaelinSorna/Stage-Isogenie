...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Require Un modèle défini sur $\F_{q}$ de courbe elliptique $E$ à multiplication complexe, le discriminant $\Delta$ de son anneaux d'endomorphisme, $l$ premier ne divisant pas $p*f_{m}$ tel que $\left( \frac{\Delta}{l}\right) = 1$
		...
		\State Appliquer la formule de Vélu pour trouver une forme explicite $\phi_1 : E \rightarrow E_1$ 
		\State Calculer l'invariant différentiel de $E_1$
		\State Calculer la constante $c$ telle que $\phi_{1}^{*}(w_{E_{1}}) = cw_E$
		\State Appliquer l'isomorphisme $\mu_{c} : (x,y) \mapsto (c^{-2}x, c^{-3}y)$ à la forme explicite de $\phi_1 : E\rightarrow E_1$ pour calculer $\phi = \mu_{c}\circ\phi_{1} : E \rightarrow E'$
		\Return La forme explicite $\phi : E \rightarrow E'$
	\end{algorithmic}
...
Comme on peut choisir $N$ de taille raisonnable, on peut donc appliquer la procédure naïve. 


\begin{algorithm}
	\caption{Famille génératrice du groupe de classe par des idéaux de Seysen}
	\begin{algorithmic}[1]
		\Require Un entier positif $N$, un discriminant $\Delta$, la caractéristique $p$, le conducteur maximal $f_{m}$.
		\Ensure Une liste $L_{premiers}$ de nombres premiers plus petit que $N$ ne divisant pas $pf_{m}$, la liste $L_{ideaux}$ de leurs idéaux de Seysen respectif, la liste $L_{formes}$ de leurs idéaux de Seysen respectif. 
		...
		\EndWhile
		\Return $L_{premiers}$, $L_{ideaux}$, $L_{formes}$
	\end{algorithmic}
\end{algorithm}
...
On énoncé ensuite les bonnes propriétés des marches aléatoires sur les groupes de classes. Sans rappeler la théorie des graphes expanseurs, on cite les résultats principaux.
...
Maintenant on revient sur la borne $N$ des nombres premiers utilisés. On peut chercher à l'optimisé par rapport à l'implémentation de l'algorithme 4.
...
Dans cette partie, on suppose connue un idéal $\LR\subset\OR_{\Delta}$ de norme $l$ premier décomposé, et une courbe elliptique $E$ définie sur $\F_{q}$ à multiplication complexe avec $End(E)\simeq\OR_{\Delta}$ normalisé. On cherche un générateur du groupe de torsion $E\left[ \LR\right]\subset E(\Fb_{p})$. On suppose $l$ premier avec le conducteur maximal $f_{m}$.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\Require Un nombre premier $l$ premier avec $p*f_{m}$, une courbe elliptique $E$ à multiplication complexe définie sur $\F_{q}$, d'anneau d'endomorphisme $\OR_{\Delta}$, un idéal propre $\LR\subset\OR_{\Delta}$ sous forme $\LR = \left[ l, \frac{c + d\pi_{q}}{f_{m}}\right]$
...
\subsubsection{Méthode d'Atkin Elkies}

La méthode 2 précédente est une net amélioration de la méthode 1, mais nécessite parfois de manipuler des groupes $E(F_{q^{r}})$ de très grand cardinal, lorsque $r$ est de l'ordre de $l-1$. En pratique, cela arrive suffisament souvent pour être problèmatique. On présente ici une troisième méthode, ayant pour but d'éviter tout calcul dans une quelconque extension de $\F_{q}$. 

Voir : Article de Schoof, article de Bostan

Article de Schoof :

L'idée de la méthode repose sur beaucoup de théorème qui seront admis et résumé. Soit $E$ une courbe elliptique à multiplication complexe de $j$-invariant $j$, définie sur $\F_{q}$.
\begin{itemize}
	\item Il existe un polynôme $\Psi_{l}(X,Y)$, dépendant uniquement de $l$, tel que les racines de $\Psi_{l}(j,Y)$ dans $\Fb_{p}$ sont exactement les $j$-invariants des $l+1$ courbes $l$-isogones à $E$.
	\item Si $l$ est décomposé, $\Psi_{l}(j,Y)$ admet uniquement $2$ racines dans $\F_{q}$, obtenues en factorisant $PGCD(\Psi_{l}(j,Y), Y^q - Y)$
	\item Soit $h$ une de ces deux racines, alors il est possible d'obtenir un modèle de la courbe $E'$ défini sur $\F_{p}$, à partir de la donnée de $E$, $h$ et $l$. 
	\item Soit $p_{1}$ la somme des abscisses des $l-1$ points finis du noyau de la $l$-isogénie $\phi : E\rightarrow E'$. Alors $p_{1}\in\F_{q}$ peut être obtenues à partir de la connaissance de $E$, $h$ et $l$., avec des calculs uniquement dans $\F_{q}$.
	\item Soit $F_{h}$ le polynôme dont les racines sont les abscisses des élément du noyau de $\phi$. Soit $\wp$ la fonction de Weierstrass associée à un relèvement de $E$ via le théorème de relèvement de Deuring. 
	\begin{equation}
		z^l-1F(\wp(z)) = \exp(S)
	\end{equation}
	où $S$ est une série dont tout les termes dépendent de l'écriture en série de Laurent de $\wp$ et de $p_{1}$
\end{itemize}

On peut alors retrouver les coefficients de $F$ en identifiants les $(l-1)/2$ premiers termes des deux membres de l'équation.

Article de Bostan : Section 4.2 méthode de Elkies 1998 : 

Ecriture plus explicite de la méthode énoncé par Schoof. Cependant la formule (17) n'a pas de sens, la notation "$p_{i}$" n'est pas définie et le calcul de $p_{2}$ dépend de $p_{0}$ qui n'a pas de sens. 


Article de Bostan : Section 6.4 méthode de Atkin : 

Même théorème mais différente approche, semble faisable à condition de pouvoir manipuler les séries de Laurent dans sage (calculer $\wp^i$ pour $i$ allant de $1$ à $l-1$ jusqu'au coefficient de degré $l$)
...
\end{equation}
Où les idéaux $\PR_{p_{i}}$ sont des idéaux de Seysen de normes $p_{i}$ différent de $p$ et ne divisant pas $f_{m}$. On cherche alors $\alpha\in\K$ tel que 
\begin{equation}
...
Ainsi lorsque $f$ est une forme quadratique binaire primitive qui représente $1$, sa forme réduite $r$ est de la forme $x^2 + bxy + cy^2$. 

\begin{Rem}
	Chaque étape de l'algorithme 2 correspond à des changement de variable en $(x,y)$ \cite{Cox}[Thm 2.8]. En effet
	\begin{enumerate}
...
\begin{Prop}
	L'algorithme 5 appliqué à un idéal $\LR$ admet une complexité en $O(\log(N(\LR)^2))$.
\end{Prop}

\begin{proof}
	Le nombre de calcul de cette algorithme et le double du nombre de calcul nécessaire à la réduction de la forme quadratique $f$. D'après l'analyse de l'algorithme $2$, on déduit la complexité de l'algorithme $5$. 
\end{proof}
...
\textbf{Supprimer la partie Cornacchia ?} 
...
\textbf{Voir méthode de sage, via équivalence de forme quadratique. méthode sage "gens.reduced()"}
