...
\subsection{Endomorphisme de tores}
...
\begin{Def}
	...
	\end{equation*}
	où $\alpha\in\left\lbrace \alpha\in\C : \alpha\Lambda_1 \subset \Lambda_2\right\rbrace $.  
	
\end{Def}

\begin{Rem}
	Si $\phi: \C/\Lambda_1 \rightarrow \C/\Lambda_2$ est une application holomorphe et nulle en $0$, alors elle de la forme $\phi_{\alpha}$ pour un certain $\alpha$. On choisit donc de définir les morphismes de tores comme étant les applications de la forme $\phi_{\alpha}$.
\end{Rem}
...
\begin{Coro}
	...
	\begin{equation*}
		\alpha\in\left\lbrace \alpha\in\C : \alpha\Lambda \subset \Lambda\right\rbrace 
	\end{equation*}
...
\subsection{Courbe elliptique à multiplication complexe}


Lorsque $E$ est une courbe elliptique à multiplications complexes, $End(E)$ peut être inclus dans $\C$ de deux manières (à conjugaison prêt dans le corps quadratique imaginaire). On souhaite normaliser le choix de l'injection de $End(E)$ dans $\C$. Pour cela, on veut associer à tout $\alpha$ tel que $\alpha\Lambda\subset\Lambda$, un endomorphisme notée $[\alpha]$. 

Si $\omega_{E}$ est un invariant différentiel de $E$, on voudra que l'application $\alpha\mapsto\left[ \alpha\right]$ soit cohérente avec l'action de $[\alpha]$ sur $\omega_{E}$. D'après le corollaire 2.16, il existe une constate $c\in\C$ telle que
\begin{equation*}
	[\alpha]^{*}(\omega_{E}) = c\omega_{E}
\end{equation*}
Comme tout les invariants différentiels de $E$ sont égaux à une constante complexe prêt, $c$ ne dépend pas du choix du modèle de $E$. On souhaite construire $\alpha\mapsto\left[ \alpha\right]$ de sorte que $c = \alpha$.
...
\begin{Prop}
	Soit $E$ une courbe elliptique sur $\C$ à multiplication complexe. Il existe isomorphisme d'anneaux
	\begin{equation*}
...
\begin{proof}
	En choisissant $\OR$ comme l'image de l'application $f$ de la proposition 3.8, la réciproque de $f$ convient.
\end{proof}
...
Cette dernière proposition permet de voir tout endomorphisme d'une corube elliptique $E$ comme un nombre complexe. On va montrer que avec cette convention de normalisation, l'image $\OR$ de $End(E)$ dans $\C$ correspond à l'anneau d'endomorphisme des tores de réseaux homothétiques associé à $E$ :


\begin{Prop}\cite{Sil94}[II 1.1]
	Soit $(E,[.])$ une courbe elliptique définie sur $\C$ à multiplication complexe normalisé. Soit $End(E)\simeq\OR\in\C$ et $\alpha\in\OR$. Si $\lambda$ est un réseau associé à $E$, alors $\alpha\Lambda\in\Lambda$ et l'isogénie $[\alpha]$ est équivalent à l'endomorphisme de tore $\phi_{\alpha}$.
	
\end{Prop}

La donnée d'un réseau associé permet d'expliciter le corps quadratiques imaginaire dont l'anneau d'endomorphisme est un ordre.
...
Dans cette section, les preuves seront rédigés.
...
Soit $E_{\Lambda}$ une courbe elliptique associée à un réseau $\Lambda$. On cherche à décrire les noyaux possibles des isogénies partant de $ E_{\Lambda}$. On supposera toujours l'isomorphisme $End(E)\simeq\OR_{\Delta}$ normalisé. Dans la suite, $\K\subset\C$ est le corps quadratiques imaginaire dont $\OR_{\Delta}$ est un ordre. 

On se pose la question suivante : Partant de $\OR_{\Delta}$, comment trouver $\Lambda$ telle que $End(E_{\Lambda}) = \OR_{\Delta}$ ? 

\begin{Def}
	Soit $\OR_{\Delta}$ un ordre de discriminant $\Delta$ d'un corps quadratique imaginaire. On note $ELL(\OR_{\Delta})$ l'ensemble des courbes elliptiques à multiplication complexes d'anneaux d'endomorphismes $\OR_{\Delta}$. 
\end{Def}


Soit $\LR$ un idéal fractionnaire de $\OR_{\Delta}$. Un tel idéal est un $\Z$-module libre de rang $2$ qui n'est pas inclus dans $\R$, donc un réseau. On peut donc lui associer une courbe elliptique $E_{\LR}$.

\begin{Prop}
	Soit $\OR_{\Delta}$ un ordre de discriminant $\Delta$ d'un corps quadratique imaginaire. Soit $\LR\subset\OR_{\Delta}$ un idéal fractionnaire propre. Alors $End(E_{\LR})\simeq\OR_{\Delta}$, donc $E_{\LR}\in ELL(\OR_{\Delta})$.
\end{Prop}
...
Puisque des réseaux homothétiques définissent des courbes équivalentes, pour tout $c\in\OR_{\Delta}$, $c\LR$ est à la fois un idéal de $\OR_{\Delta}$ et un réseau homothétique à $\LR$. On s'intéresse donc aux classes d'idéaux : Soit $Cl(\OR_{\Delta})$ le groupe de classes de $\OR_{\Delta}$, et $\CL$ la classe de $\LR$. 

L'application $Cl(\OR_{\Delta})\rightarrow ELL(\OR_{\Delta})$, $\CL\mapsto E_{\LR}$ est donc bien définie. On va montrer qu'il s'agit d'une bijection

\begin{Def}
	Soit $\Lambda$ un réseau et $\LR$ un idéal fractionnaire de $\OR_{\Delta}$. On défini le produit $\LR\Lambda$ comme l'ensemble 
	\begin{equation*}
...
\begin{Prop}
	Soit $\Lambda$ un réseau et $\LR$ un idéal fractionnaire de $\OR_{\Delta}$. Alors $\LR\Lambda$ est un réseau de $\C$. De plus, si $E_{\Lambda}\in ELL(\OR_{\Delta})$ alors $E_{\LR\Lambda}\in ELL(\OR_{\Delta})$
\end{Prop}
...
Soit $\Lambda$ un réseau tel que $End(E_{\Lambda}) \simeq \OR_{\Delta}$. Pour tout idéal fractionnaire propre, $End(E_{\LR\Lambda}) = End(E_{\Lambda})$, mais $\Lambda$ et $\LR\Lambda$ ne sont pas à priori homothétique. $E_{\Lambda}$ et $E_{\LR\Lambda}$ sont donc deux éléments distincts de $ELL(\OR_{\Delta})$. 
...
\begin{proof}\cite{Sil94}[II,1.2]
	...
	\begin{enumerate}
		\item On sait que la normalisation de $End(E)$ implique que $\OR_{\Delta}\Lambda = \Lambda$. De plus $E_{\LR\Lambda} \simeq E_{\LR'\Lambda}$ si et seulement si $\LR\Lambda = c\LR'\Lambda$ avec $c\in\C^{*}$. $\LR$ et $\LR'$ étant inversible, cela équivaut à 
		\begin{equation*}
		...
		\item On se donne deux réseaux $\Lambda_1$ et $\Lambda_2$ dont les courbes elliptiques associées sont dans $ELL(\OR_{\Delta})$, et on cherche $\CL$ tel que $\CL\cdot E_{\Lambda_1} = E_{\Lambda_2}$. Cela équivaut à trouver un idéal fractionnaire $\LR$ tel que $\Lambda_2$ et $\LR^{-1}\Lambda_1$ soient homothétiques. Mais puisque $E_{\Lambda_1}\in ELL(\OR_{\Delta})$, $\OR_{\Delta}\Lambda_1 = \Lambda_1$ et 
		\begin{equation*}
...
Notons que l'on peut normaliser toutes les courbes elliptiques de $ELL(\OR_{\Delta})$ de façon cohérente.
	 
\begin{Prop}
	Soit $(E_1,[.]_1)$ et $(E_2,[.]_2)$ deux courbes elliptiques définient sur $\C$, à multiplication complexe, normalisées et de même anneaux d'endomorphisme $\OR_{\Delta}$. Soit $\phi : E_1 \rightarrow E_2$ une isogénie. Alors $\forall\alpha\in\OR_{\Delta}$¨
	\begin{equation*}
...
Si $\phi : E \rightarrow E'$ est une isogénie entre deux courbes elliptiques $E$, $E'$ distinctes de même anneaux d'endomorphisme $\OR_{\Delta}$, alors $\phi$ correspond à l'action d'un unique élément $\CL\in C(\OR_{\Delta})$ sur $E$, puisque l'action est simplement transitive. On ne peut cependant pas déterminer $\phi$ à partir de $\CL$, car $\CL$ défini $\phi$ à endomorphisme prêt au départ et à l'arrivé. 
...
On se donne une courbe elliptique $E\in Ell(\OR_{\Delta})$ et un idéal fractionnaire $\LR$. On souhaite définir à partir de $\LR$ une isogénie $\phi : E\rightarrow \CL\cdot E$ qui va nous permettre de définir une isogénie $\phi$. Pour cela, on détermine à partir de $\LR$ le noyau de $\phi$. Puisque toute classe $\CL$ contient un représentant entier, on peut supposer $\LR$ entier dans un premier temps.

\begin{Def}
	Soit $(E,[.])$ une courbe elliptique définie sur $\C$ à multiplication complexe par $\OR_{\Delta}$, normalisée. Soit $\LR\in\OR_{\Delta}$ un idéal entier propre. Le groupe de $\LR$-torsion de $E$ est 
	\begin{equation*}
