...
\begin{Rem}
	Comme les nombres premiers à éviter sont en nombre fini, les conditions que l'on s'impose ne change pas le comportement asymptotique de la borne $N$.
\end{Rem} 
...
\textbf{Estimation de la consante C ?}
...
\begin{Rem}
	L'étape 1 nécessite d'être détaillée pour analysé la complexité. Nous y reviendrons à la section 5.7
\end{Rem}
...
Étant donné un modèle de courbe elliptique $E$, les formules de Vélus permettent de construire une forme explicite d'une isogénie $\phi$ à partir de la connaissance de son noyau. Pour une $l$-isogénie, le noyau peut être donné sous deux formes : un point $K$ générateur du noyau, ou un polynôme $F$ dont les racines sont les abscisses des points finis du noyau. Dans le second cas, il faut adapter les formules pour les écrire en fonction de $F$.
...
Dans les deux cas, \cite{Velusqrt} propose une méthode pour évaluer des polynômes de degré $l$ dans $\F_{q}[X]$ en $O(\sqrt{l})$ opération dans $\F_{q}$. Les résultats suivants en découle.
