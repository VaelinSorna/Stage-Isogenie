...
Dans $\Z$, l'idéal engendré par un nombre premier $p$ est un idéal premier. Cependant dans un ordre $\OR$ d'un corps de nombre, l'idéal $p\OR$ associé à $p$ peut ne plus être premier. 

On regroupe ici des résultats décrivant les différentes possibilités pour le comportement de $p$ vu dans $\OR$ :

\begin{Def}
	Soit $\OR$ ordre de conducteur $f$, et $p$ premier ne divisant pas $f$. On note $p\OR = \PR_{1}^{e_{1}}\PR_{2}^{e_{2}}\ldots\PR_{g}^{e_{g}}$ la factorisation de $p\OR$ dans $I(\OR,f)$. 
