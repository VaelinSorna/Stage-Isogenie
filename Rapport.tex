...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\State Appliquer l'isomorphisme $\mu_{c} : (x,y) \mapsto (c^{-2}x, c^{-3}y)$ à la forme explicite de $\phi_1$ pour calculer $\phi = \mu_{c}\circ\phi_{1}$
	\end{algorithmic}
...
ALGO 5 CALCUL D'UN GENERATEUR DE GROUPE DE TORSION, VERSION 1
...
En pratique sur l'exemple "small" de \cite{BrokerCharlesLauter}, $l = 31$ mais le calcul prend déjà quelques secondes. Ils proposent une autre méthode, utilisant la théorie des formes modulaires. On propose ici une amélioration de la première méthode : le but est de trouver le générateur du noyau directement dans la plus petite extension qui le contient, sans calculer de polynôme de division.\\
...
Puisque $l$ est supposé décomposé et premier avec $f_{m}$, on sait que l'on se trouve dans le cas où il existe exactement $2$ isogénies définies sur $\F_{q}$. On note $v_{1}$ et $v_{2}$ les deux valeurs propres de $\pi_{q}$. 
...
On sait donc directement dans quelle extension chercher les générateurs des noyaux. On sait aussi que celui qui nous intéresse est un vecteurs propre de $\pi_{q}$ pour la valeurs propres $-c$. 




ALGO 5 CALCUL D'UN GENERATEUR DE GROUPE DE TORSION, VERSION 2 
...
On Cherche plutôt à obtenir $\beta$.  La donnée du problème est la forme standard de $\beta\OR_{\Delta}$ obtenue à l'équation $(3)$. L'idéal n'est pas à priori primitif, mais il suffit de trouver un générateur de l'idéal primitif associé. On se ramène donc au problème suivant : étant donné un idéal entier primitif principal $(1,a,b)$, trouver un générateur $\beta$. Comme $N((1,a,b)) = a = N(\beta)$, connaître $a$ donne la norme de $\beta$. On sait que $\beta = \frac{x + y \sqrt{\Delta}}{2}$ avec $x$, $y$ entiers.
...
ALGO 7 ALGORITHME DE CORNACCHIA
