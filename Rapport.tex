...
Alors $ End(E) = \OR_{\Delta} $ peut être normalisé par la méthode de la section 4 : on choisit la racine de $d_{\K}$ de sorte que $\forall\beta\in\OR_{\Delta} ,\; [\beta]^{*}(w) = \beta w$. L'endomorphisme $\alpha$ a le même polynôme caractéristique que $\pi_{q}$ et peut donc être représenté par $\alpha = \frac{t + f_{m}\sqrt{d_{\K}}}{2} \in\OR_{\Delta}\subset\C$.
...
est bien définie, et en notant $\omega$ l'invariant différentiel de $E$, comme $\pi_{q}\in\PR$ :

\begin{equation*}
	\widetilde{ \left[ \frac{x + y\pi_{q}}{f_{m}}\right] } ^{*} (\omega) = \widetilde{\left( \frac{x}{f_{m}}\right)} \omega
\end{equation*}

D'après le théorème de Kummer, $\PR$ étant un idéal contenant $p\OR_{\Delta}$, les entiers qu'il contient sont exactement les multiples de $p = N(\PR)$. Donc si $f_{m}$ est premier avec $p$ : $\widetilde{\left( \frac{x}{f_{m}}\right)} = xf_{m}^{-1} [p]$.

Cette dernière équation pourra servir à normaliser dès que $p$ ne divise pas $f_{m}$. Dans le cas contraire la réduction est quand même bien défini \textbf{(?)}. 

On peut remarquer que la condition du théorème $6.7$ pour que la réduction conserve l'anneau d'endomorphisme implique déjà que $p$ est premier avec $f$. 
