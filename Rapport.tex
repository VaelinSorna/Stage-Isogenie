...
On souhaite utiliser la théorie précédente pur établir une procédure de calcul d'isogénie plus rapide que la méthode naïve donnée par la formule de Vélu. Plus précisément on suppose connaître 
\begin{enumerate}
	\item Une courbe elliptique $E$ ordinaire, donnée par une équation sur un corps $\F_{q}$ de caractéristique $p$.
	\item L'anneau d'endomorphisme $End(E)$ donné par son discriminant $\Delta$. On connaît donc le corps quadratique imaginaire $\K$ et le conducteur $f$
	\item Un nombre premier $l$ différent de la caractéristique $p$.
\end{enumerate} 
Et on cherche une courbe elliptique $E'$ définie sur $\F_{q}$ telle qu'il existe une $l$-isogénie séparable $\phi : E\rightarrow E'$ définie sur $\F_{q}$. De plus étant donné $n\in\N$, et $P$ un point de $E\left( \F_{q^{n}}\right)$ on souhaite pouvoir calculer $\phi(P)$. 
...
\begin{enumerate}
	\item Sous cette condition, toutes les $l$-isogénies séparables sont horizontales. On peut donc trouver leurs noyaux en calculant un groupe de torsion puis appliquer la formule de Vélu.
	\item On peut même représenter une isogénie uniquement par l'idéal qui engendre son noyau. Un tel idéal peut être donné sous la forme $\left[ l, \frac{c + d\pi_{q}}{f_{m}}\right]$ donc seulement déterminé par $l$, $c$ et $d$, trois entier qui en pratique seront de $l$ bits. On peut représenter une $l$-isogénie en $O(log(l))$ bits. 
	\item Si on suppose seulement que $l$ ne divise pas $f$, on peut tout de même représenter les isogénies horizontales de cette manière mais pas les isogénies descendantes. Ainsi si $l$ ne divise pas $f_{m}$, on peut représenter toutes les $l$-isogénies définies sur $\F_{q}$ en $O(log(l))$ bits.
	\item $f_{m}$ sera inversible modulo $l$, ce qui sera utile par la suite pour manipuler des $l$-isogénies représentées sous la forme $\left[ \frac{c + d\pi_{q}}{f_{m}}\right] $.
	
\end{enumerate}

On déduit une procédure naïve à partir de la formule de Vélu :

ALGO 1 : VELU AVEC TORSION
\begin{enumerate}
...
Cependant la formule de Vélu demande de parcourir le noyau entier : la procédure est linéaire en $l$, ce qui n'est pas satisfaisant. L'algorithme de Broker, Charle et Lauter consiste à améliorer cette procédure en se ramenant à des idéaux de petites normes. On le présente de la façon suivante :

\begin{itemize}
	\item On a donc besoin d'outils pour calculer dans le groupe de classe $Cl(\OR_{\Delta})$ : on étudie cela dans la section 5.2. On utilise des formes quadratiques pour manipuler les classes d'idéaux, et les exprimer dans une familles génératrices données.
	\item Partant d'un idéal $\LR$ de norme $l$, on pourra donc exprimer sa classe $\CL$ dans une famille génératrice bien choisie. On en déduit une expression de $\LR$ à un idéal fractionnaire principal prêt. Il faudra donc savoir trouver un générateur d'un idéal principal, ce que l'on étudiera en section 5.5.
	...
	\item On appliquera Velu à partir d'un générateur d'un groupe de torsion. Dans la section 5.4 on explique comment obtenir un générateur d'un groupe de torsion d'un idéal de norme premier.
...
\begin{enumerate}
	\item Pour manipuler une classe il faut définir un représentant normal, unique dans chaque classe
	\item On voudra calculer des isogénie associée aux idéaux représentant les générateurs d'une famille génératrice. Il faut dont que leurs normes vérifient les mêmes conditions fixées sur $l$ : On veut pouvoir manipuler une famille génératrice définie par des représentants de normes premier bien choisis. 
\end{enumerate}
...
\begin{Prop}
	Soit $\LR = (m,a,b)\subset\OR_{\Delta}$ un idéal entier sous forme standard. Alors
	\begin{enumerate}
		\item $N(\LR) = m^2a$
		\item $\LR^{-1} = \left( \frac{m}{N(\LR)},a,-b\right) $
	\end{enumerate}
\end{Prop}
...
\begin{Rem}
	Une forme quadratique réduite $f = (a,b)$ vérifie $a,|b| \leq \sqrt{|\Delta|}$. On peut donc représenter chaque classe $Cl(\OR_{\Delta})$ en $O(\log(|\Delta|))$ bits.
\end{Rem}

De plus à partir d'un représentant $\LR$ (ou $f$) quelconque d'une classe, il existe un algorithme donnant l'élément réduit de la classe en $\OR(\log(\Delta)^{2} )$.

ALGO 2 : REDUCTION DE FORME QUADRATIQUE (BIASSE)
...
Ainsi, toute classe d'idéaux possède un représentant premier avec le conducteur maximal. Comme il y a deux choix d'idéaux de norme $m$ lorsque $m$ est décomposé, on va fixer une convention nous permettant de choisir. Cette convention sera aussi utile pour les calculs dans le groupe de classe.
...
On veut former une famille génératrice de $Cl(\OR_{\Delta})$ définie avec un système de représentant formé uniquement d'idéaux de Seysen de norme $m$ adaptée au calcul d'isogénie, c'est à dire $m$ différent de la caractéristique $p$ et ne divisant pas le conducteur maximal $f_{m}$. Une idée naïve est d'établir la liste croissante des nombres premiers acceptables plus petit qu'une borne $N$. Pour $N$ suffisamment grand on peut espérer avoir au moins un représentant de chaque classe parmi les idéaux de Seysen associés.
...
\begin{Rem}
	Pour estimer la borne, Seysen cite deux résultats proposant $C= 280$ ou même $C = 2$. Cohen proposait $C = 6$. Cependant d'après \cite{Coh00}, en pratique cette borne $N$ est pessimiste et l'on conjecture une borne en $O(\log(\Delta)^{1+\epsilon})$ pour tout $\epsilon > 0$.
\end{Rem}
...
ALGO 3 FAMILLE GENERATRICE DE SEYSEN
...
Dans cette section on ne cherche pas à expliciter $\alpha$ mais seulement une liste d'exposant $e_{i}$ fournissant une factorisation de $\CL$ lorsque les représentants $\LR_{i}$ sont choisit parmi les idéaux de Seysen. Une idée naïve consiste à chercher de tels exposants au hasard. Deux faits vont permettre de rendre cela effectif :
...
\begin{Coro}
	Avec les notations du théorème précédent, si $a$ se factorise entièrement avec des premiers $p_{i}\in L_{pi}$, alors $\CL$ est facilement factorisable connaissant la factorisation de $a$. 
\end{Coro}

\begin{Rem}
	Pour une forme quadratique réduite $(a,b)$, on sait que $a < |\Delta|$. Factoriser $a$ est donc au pire des cas sous-exponentielle en $\log(a) \leq \log(|\Delta|)$. De plus il suffit de vérifier que $a$ se factorise complètement parmi une famille de petit nombres premiers.
\end{Rem}


ALGO 4 FACTORISATION D'UNE CLASSE 
...
Les bonnes propriétés des marches aléatoires sur les groupes de classe rendent cette procédure envisageable. Sans rappeler la théorie des graphes expanseurs, on cite le résultat principal :
